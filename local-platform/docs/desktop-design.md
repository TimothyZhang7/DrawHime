...
0.1.8 已将安装和卸载界面切换为系统语言优先的中英文文案，并把卸载数据策略改为显式“保留”选项。Windows 11 实机使用 5,000 个模拟 Runtime 文件验证默认卸载在 2 秒内返回且后台清理，不再逐文件阻塞界面；`/KEEPDATA` 卸载后 5,641 个既有文件保持完整，重新安装继续复用原目录。
