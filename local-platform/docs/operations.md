...
本项目日常修复默认只更新开发测试环境，不提交 Git、不触发 GitHub、不部署生产。任何步骤失败都停在当前步骤，修正并只重跑失败步骤后再继续。
...
4. **最小验证**：开发迭代只运行受影响包的 `type-check`、目标测试或一次必要构建；同一差异不得重复等价检查。
5. **开发测试更新**：桌面源码变化只构建一次生产协议无安装包 EXE，精确覆盖 `E:\DrawHimeStorageTest\drawhime-desktop.exe` 并保留 `data`；纯文档变化跳过构建。服务端只有存在真实且已配置的开发端点时才部署，禁止把生产部署命令当作开发发布。
6. **目标验收**：只执行受影响页面或调用链的专项验收，确认状态、性能和数据保留；不为日常修复运行 NSIS、完整发布门禁或 GitHub Actions。
7. **结束**：报告改动、最小验证和开发测试结果。没有当前任务的明确授权时，到此结束，禁止提交、推送和发布。

## 显式授权动作

- **提交**：只有用户在当前任务明确要求提交时，才用明确路径暂存、检查暂存差异并创建本地提交。
- **公开同步**：只有当前任务明确要求推送或同步公开仓库时，才复制已验证文件、比较一致性、提交并推送。
- **生产部署**：只有当前任务明确要求部署生产时，才执行生产预检、目标 `--dry-run`、正式部署及生产验收。
- **桌面发布**：只有当前任务明确要求发布时，才执行版本、签名、NSIS、更新清单、在线升级验收、GitHub Actions 或 GitHub Release。
- 历史任务中的“提交”或“发布”授权不得自动延续到新一轮修复。
...
- 默认不创建本地提交、不同步公开仓库；只有当前任务明确授权时才追加这些步骤。
- 纯文档任务只执行 Markdown 检查与 `git diff --check`，不构建、不部署、不提交、不推送。
...
文档独立修改不重启运行服务，只完成链接、Markdown 与 diff 检查。训练 Runtime 只有在用户明确要求生产部署时，才使用 `node scripts/deploy-training-runtime.mjs --dry-run` 后正式部署，不与平台部署命令混用。
