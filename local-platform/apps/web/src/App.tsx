...
function JobHistoryCard({ job, sessionToken, onDetail, onCancel, onDelete }: { job: InferenceJobView; sessionToken: string; onDetail: (job: InferenceJobView) => void; onCancel: (job: InferenceJobView) => void; onDelete: (job: InferenceJobView) => void }) {
  ...
  return <article className="card job-card"><div className="job-card-cover">{artifact ? <ArtifactImage artifact={artifact} token={sessionToken} /> : <div className="job-placeholder">{!isFinal(job.status) ? <LoaderCircle className="spin" /> : <ImageIcon />}</div>}<div className="job-card-cover-top"><span className={`status ${job.status}`}>{statusLabel(job.status)}</span>{job.loras.length > 0 && <b>{job.loras.length} LoRA</b>}</div><span className="job-card-cover-size">{dimensions}</span></div><div className="job-card-copy"><time>{createdAt}</time><h3 title={job.requestedPrompt}>{job.requestedPrompt || "未填写提示词"}</h3><p title={job.modelDisplayName}>{job.modelDisplayName}</p><div>{job.publication && <span>图库 · {publicationStatusLabel(job.publication.status)}</span>}{job.source === "bot" && <span>Bot</span>}</div>{job.errorMessage && isFinal(job.status) && <em title={job.errorMessage}>{job.errorMessage}</em>}</div><TaskActions job={job} onDetail={onDetail} onCancel={onCancel} onDelete={onDelete} /></article>;
}
...
function statusLabel(status: InferenceJobView["status"]): string {
  return { queued: "排队中", reserving: "预留余额", ready: "等待 GPU", running: "生成中", succeeded: "已完成", failed: "失败", cancelled: "已取消" }[status];
}
