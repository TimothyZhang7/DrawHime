 ...
 */
import { resolveTrainingCycles, type InferenceModelView, type TrainingCaptionStageView, type TrainingDatasetView, type TrainingJobView, type TrainingParameters, type TrainingPriceQuoteView } from "@drawhime/contracts";
...
function TrainingCreator({ token, dataset, models, onCreated }: { token: string; dataset: TrainingDatasetView; models: InferenceModelView[]; onCreated: () => Promise<void> }) {
  ...
  return <section className={`card training-stage-card training-create${confirmed ? "" : " is-locked"}`}>
    ...
    <fieldset disabled={!confirmed || busy}>
      ...
      <label className="field"><span>触发词 <small>逗号分隔；Runtime 会临时前置，已确认 Caption 不会被改写</small></span><input value={triggers} onChange={(event) => setTriggers(event.target.value)} placeholder="例如：my_character" /></label>
...
/** 按数据集规模生成稳定默认参数；320 次以上遍历来自生产耗时与角色收敛结果的折中。 */
function defaultTrainingParameters(assetCount: number): TrainingParameters {
  ...
  const { epochs, repeats } = resolveTrainingCycles(safeAssetCount, 320);
