...
import { ArrowLeft, BookOpenText, Check, ChevronRight, CircleDollarSign, Cpu, ExternalLink, FileImage, FilePlus2, Gauge, ImageIcon, Images, LoaderCircle, Pencil, Plus, Search, Settings2, SlidersHorizontal, Sparkles, Trash2, Upload, X } from "lucide-react";
...
function ModelGalleryCard({ entry, token, onOpen }: { entry: ModelLibraryEntryView; token: string; onOpen: () => void }) {
  return <button className="model-gallery-card" onClick={onOpen}>
    <div className="model-gallery-cover"><ModelExampleImage example={entry.examples[0]} token={token} alt={entry.displayName} /><span>{entry.familyName}</span><b>¥{entry.priceCny} / 张</b></div>
...
function ModelDetailPage({ session, entry, loading, message, onBack, onUseModel, onChanged }: { session: LocalPlatformSessionView | null; entry: ModelLibraryEntryView | null; loading: boolean; message: string; onBack: () => void; onUseModel: (modelVersionId: string) => void; onChanged: (entry: ModelLibraryEntryView, notice: string) => Promise<void> }) {
  ...
  return <div className="model-detail-page">
    ...
    <section className="card model-detail-hero">
      ...
      <div className="model-detail-summary"><div className="model-detail-badges"><span>{entry.familyName}</span>{!entry.visible && <span className="hidden">仅管理员可见</span>}<span>¥{entry.priceCny} / 张</span></div><h1>{entry.displayName}</h1><p>{entry.description}</p><dl><div><dt><Cpu />GPU 文件</dt><dd>{entry.modelFileName}</dd></div><div><dt><Gauge />推荐采样</dt><dd>{entry.parameters.steps} 步 · CFG {entry.parameters.cfg}</dd></div><div><dt><Settings2 />采样器</dt><dd>{entry.parameters.sampler} · {entry.parameters.scheduler}</dd></div><div><dt><ImageIcon />采样最长边</dt><dd>{entry.parameters.samplingMaxEdge}px / 输出 {entry.parameters.maxEdge}px</dd></div></dl></div>
    </section>
    <section className="card model-usage-guide"><header><BookOpenText size={17} /><div><span>使用说明</span><h2>如何获得稳定效果</h2></div>{entry.sourceUrl && <a href={entry.sourceUrl} target="_blank" rel="noreferrer"><ExternalLink size={14} />模型来源</a>}</header><p>{entry.usageGuide}</p></section>
    ...
    <section className="card model-reference-section"><header><div><span>公开引用</span><h2>使用示例 <b>{entry.referenceTasks.length}</b></h2></div><small>仅展示使用该底模、已发布到主站图库的公开任务</small></header>{entry.referenceTasks.length ? <div className="model-reference-grid">{entry.referenceTasks.map((task) => <a key={task.id} href={`/image/${task.galleryItemId}`}><img src={task.imageUrl} alt="模型公开使用示例" loading="lazy" /><strong>{task.prompt}</strong><small>{new Date(task.createdAt).toLocaleDateString("zh-CN")}</small><ExternalLink size={13} /></a>)}</div> : <div className="model-example-empty"><Images size={28} /><span>暂时没有已公开的使用示例。</span></div>}</section>
...
function ModelEditDialog({ token, entry, onClose, onSaved }: { token: string; entry: ModelLibraryEntryView; onClose: () => void; onSaved: (entry: ModelLibraryEntryView) => Promise<void> }) {
  const [form, setForm] = useState<ModelLibraryUpdateRequest>({ displayName: entry.displayName, description: entry.description, sourceUrl: entry.sourceUrl, usageGuide: entry.usageGuide, visible: entry.visible });
  const [busy, setBusy] = useState(false);
  const [error, setError] = useState("");
  ...
  return <div className="modal-backdrop model-dialog-backdrop">
    <section className="task-dialog model-dialog model-edit-dialog" role="dialog" aria-modal="true" aria-label="编辑底模">
      ...
      <div className="model-dialog-body">
        ...
        <label className="field"><span>模型来源链接 <small>可选</small></span><input value={form.sourceUrl || ""} onChange={(event) => setForm({ ...form, sourceUrl: event.target.value.trim() || null })} /></label>
