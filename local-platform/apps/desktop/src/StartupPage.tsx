...
export function StartupPage({ state, catalog, progress, installProgress, phase, checking, bulkBusy, onPrimary, onRecheck, onOpenQueue, onInstallRequired, onDownload, onPause, onInstall }: StartupPageProps) {
  ...
  return <div className="desktop-page startup-page">
    ...
    <div className="startup-detail-grid">
      ...
      <section className="startup-panel environment-panel"><header><div><span>DEVICE CHECK</span><h2>当前电脑</h2></div><button disabled={checking || Boolean(phase)} onClick={onRecheck}>{checking ? <LoaderCircle className="spin" /> : <RefreshCw />}重新检测</button></header><div className="startup-device"><div className="startup-gpu"><Cpu /><span><small>图形设备</small><strong>{gpu?.name || adapters[0]?.name || "未检测到图形设备"}</strong><em>{adapters.length ? adapters.map((adapter) => `${adapter.vendor} · ${adapter.cudaSupported ? "CUDA 可检测" : "不支持当前 CUDA Runtime"}`).join(" / ") : "厂商未知"}</em></span></div><dl><div><dt><MemoryStick />显存</dt><dd>{gpu ? `${formatBytes(gpu.memoryFreeBytes)} / ${formatBytes(gpu.memoryTotalBytes)}` : "-"}</dd></div><div><dt><Cpu />处理器</dt><dd>{state.environment.cpu.logicalCores} 线程</dd></div><div><dt><HardDrive />系统内存</dt><dd>{formatBytes(state.environment.memory.totalBytes)}</dd></div></dl></div><div className="startup-compatibility"><AlertTriangle /><span>部分功能、Runtime 和模型仅支持 NVIDIA 显卡，AMD 显卡可能无法使用或需要不同运行环境，请先确认硬件兼容性。训练集整理、手动标签和仓库浏览不受此限制。</span></div>{blockingIssue ? <div className="startup-blocking"><AlertTriangle /><div><strong>{blockingIssue.title}</strong><span>{blockingIssue.message}</span></div></div> : <div className="startup-check-ok"><CheckCircle2 /><span>硬件和系统满足当前启动条件</span></div>}</section>
    </div>
  </div>;
}
