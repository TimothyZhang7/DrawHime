/**
 * 本文件实现质量优先的本地生成表单、三档预设、专业参数说明与高级参数折叠区。
 */
...
import { CheckCircle2, ChevronDown, Database, Gauge, Layers3, LoaderCircle, Play, SlidersHorizontal, Sparkles, Zap } from "lucide-react";
...
const PRESETS: Array<{ id: PresetName; title: string; summary: string; detail: string; Icon: typeof Zap }> = [
  { id: "fast", title: "快速", summary: "20 步 · 约 0.8 MP 采样", detail: "适合构图测试、提示词迭代与 LoRA 强度试验。", Icon: Zap },
  { id: "quality", title: "质量", summary: "37 步 · 约 1.35 MP 采样", detail: "默认档，与当前 GPU 服务器平衡质量参数一致。", Icon: Sparkles },
  { id: "extreme", title: "极致", summary: "45 步 · 约 2.07 MP 采样", detail: "优先细节和材质，显存占用与生成时间最高。", Icon: Gauge },
];
const STANDARD_SIZES = ["1024x1024", "1024x1536", "1536x1024", "864x1536", "1536x864"];

/** 本地生成页在页面切换时保持表单实例，任务提交后立即交给 SQLite 队列。 */
export function GenerationPage({ models, loras, inferenceReady, defaultPrivacy, onCreated, onError }: GenerationPageProps) {
  ...
  /** 独立修改模型或文本编码器强度，用于专业用户精确控制 LoRA 影响。 */
  ...
  return <div className="desktop-page generate-layout"><section className="section-card generation-form"><header><div><span>LOCAL GENERATION</span><h2>本地生成</h2></div><small>{inferenceReady ? "质量优先 · 参数随任务固化 · 后台串行执行" : "GPU、Runtime 或底模未就绪，任务提交已锁定"}</small></header>{availableModels.length === 0 ? <div className="resource-unconfigured"><Database /><div><strong>尚无可用底模</strong><span>请先前往模型仓库安装底模，或导入已有 safetensors。</span></div></div> : <>
    <section className="generation-presets" aria-label="生成质量预设">{PRESETS.map(({ id, title, summary, detail, Icon }) => <button key={id} type="button" className={form.qualityPreset === id ? "active" : ""} onClick={() => choosePreset(id)}><Icon /><span><strong>{title}</strong><small>{summary}</small><em>{detail}</em></span>{form.qualityPreset === id && <CheckCircle2 />}</button>)}</section>
    {form.qualityPreset === "custom" && <div className="generation-custom-notice"><SlidersHorizontal /><span><strong>自定义参数</strong><small>已保留你的专业设置；点击任一预设可恢复经过验证的整套参数。</small></span></div>}
    ...
    <details className="generation-advanced"><summary><span><SlidersHorizontal /><b>高级采样设置</b><small>采样预算、极端画幅和质量提示控制</small></span><ChevronDown /></summary><div className="generation-advanced-grid">
      <ParameterField label="自定义输出宽度" help="64–2048 的 8 倍数。只改变成图尺寸，不自动改变质量预设。"><input type="number" min={64} max={2048} step={8} value={form.width} onChange={(event) => setForm((current) => ({ ...current, width: Number(event.target.value) }))} /></ParameterField>
      <ParameterField label="自定义输出高度" help="64–2048 的 8 倍数。可与宽度组合任意画幅，极端比例会使用独立步数。"><input type="number" min={64} max={2048} step={8} value={form.height} onChange={(event) => setForm((current) => ({ ...current, height: Number(event.target.value) }))} /></ParameterField>
      ...
      <ParameterField label="输出缩放算法" help="采样尺寸与输出尺寸不同时使用。Lanczos 细节保留最好，Bicubic 更柔和。"><select value={form.upscaleMethod} onChange={(event) => changeProfessional("upscaleMethod", event.target.value as DesktopLocalJobCreateInput["upscaleMethod"])}><option value="lanczos">Lanczos</option><option value="bicubic">Bicubic</option><option value="bilinear">Bilinear</option><option value="area">Area</option><option value="nearest-exact">Nearest Exact</option></select></ParameterField>
    ...
    {availableLoras.length > 0 && <section className="generation-loras"><header><div><strong>叠加 LoRA</strong><span>最多 4 个；模型强度控制画面影响，CLIP 强度控制触发词影响</span></div><b>{form.loras.length}/4</b></header><div>{availableLoras.map((lora) => { const selection = form.loras.find((item) => item.id === lora.id); return <article key={lora.id} className={selection ? "selected" : ""}><button type="button" onClick={() => toggleLora(lora.id)}><i>{selection ? <CheckCircle2 /> : <Layers3 />}</i><span><strong>{lora.title}</strong><small>{loraTypeLabel(lora.type)} · {lora.triggerWords.join(", ") || "无触发词"}</small></span></button>{selection && <div className="generation-lora-strengths"><label><span>模型强度</span><input type="number" min={-2} max={2} step={0.05} value={selection.strength} onChange={(event) => changeLoraStrength(lora.id, "strength", Number(event.target.value))} /><small>控制 LoRA 对画面结构和风格的影响。</small></label><label><span>CLIP 强度</span><input type="number" min={-2} max={2} step={0.05} value={selection.clipStrength} onChange={(event) => changeLoraStrength(lora.id, "clipStrength", Number(event.target.value))} /><small>控制触发词与文本条件的影响。</small></label></div>}</article>; })}</div></section>}
    <label className="prompt-field"><span>提示词</span><textarea value={form.prompt} onChange={(event) => setForm((current) => ({ ...current, prompt: event.target.value }))} placeholder="描述主体、画风、构图、光影和细节；支持 Anima 标签或自然语言。" /><small>提示词会原样固化；开启质量前缀时只在 Runtime 执行前补齐缺失质量标签。</small></label>
    <label className="prompt-field negative"><span>负面提示词</span><textarea value={form.negativePrompt || ""} onChange={(event) => setForm((current) => ({ ...current, negativePrompt: event.target.value || null }))} placeholder="可选；留空时可使用模型级默认负面词。" /><small>始终通过独立 negative conditioning 进入工作流，不与正面提示词合并。</small></label>
    <footer><div><strong>{presetLabel(form.qualityPreset)}</strong><span>{form.steps} 步 · CFG {form.cfg} · {form.samplerName} / {form.schedulerName}</span></div><button disabled={busy || !inferenceReady || !form.modelId || !form.prompt.trim()} onClick={() => void submit()}>{busy ? <LoaderCircle className="spin" /> : <Play />}{busy ? "正在创建任务" : inferenceReady ? "提交本地任务" : "等待环境就绪"}</button></footer>
  </>}</section></div>;
}

/** 参数字段统一展示名称、控件和面向新用户的解释。 */
function ParameterField({ label, help, children }: { label: string; help: string; children: ReactNode }) { return <label className="generation-parameter"><span>{label}</span>{children}<small>{help}</small></label>; }

/** 高级布尔参数使用可点击整行的稳定开关。 */
function SwitchField({ label, help, checked, onChange }: { label: string; help: string; checked: boolean; onChange: (checked: boolean) => void }) { return <label className="generation-switch"><span><strong>{label}</strong><small>{help}</small></span><input type="checkbox" checked={checked} onChange={(event) => onChange(event.target.checked)} /><i /></label>; }
