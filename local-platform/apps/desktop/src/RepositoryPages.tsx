...
function modelGroupProgress(resources: ResourceItem[], downloads: Record<string, DesktopResourceDownloadView>, installs: Record<string, DesktopResourceInstallView>): { percent: number | null; label: string } {
  ...
  if (installing) return { percent: Math.round(installs[installing.id]!.progress), label: "正在安装" };
  const downloading = resources.find((resource) => ["queued", "downloading", "verifying"].includes(downloads[resource.id]?.status));
  if (downloading) { const value = downloads[downloading.id]!; return { percent: Math.round(value.downloadedBytes / Math.max(1, value.totalBytes) * 100), label: value.status === "verifying" ? "正在校验" : "正在下载" }; }
  const installed = resources.filter((resource) => resource.installed).length;
  return { percent: installed ? Math.round(installed / resources.length * 100) : null, label: installed === resources.length ? "安装完成" : "等待下载" };
}
