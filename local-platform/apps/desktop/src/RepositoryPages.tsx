...
function ModelImportDialog({ onClose, onImported, onError }: { onClose: () => void; onImported: (model: DesktopLocalModelView) => void; onError: (message: string) => void }) {
  ...
  return <RepositoryDialog title="添加 Anima 底模" description="选择一个 Anima 主模型即可；已初始化的 Qwen 文本编码器和 VAE 会自动复用。" onClose={onClose}><div className="repository-model-import"><FileField label="Anima 底模文件" value={form.modelSourcePath} onPick={() => void chooseModel()} /><label><span>模型名称</span><input value={form.displayName} onChange={(event) => setForm({ ...form, displayName: event.target.value })} placeholder="选择文件后自动填写" /></label><button type="button" className={`repository-advanced-toggle ${advanced ? "active" : ""}`} onClick={() => setAdvanced((value) => !value)}><Settings2 />高级组件<ChevronRight /></button>{advanced && <div className="repository-advanced-fields"><p>留空时复用初始化安装的共享组件；自定义时必须同时选择两项。</p><FileField label="自定义文本编码器" value={form.textEncoderSourcePath || ""} onPick={() => void choose("textEncoderSourcePath")} /><FileField label="自定义 VAE" value={form.vaeSourcePath || ""} onPick={() => void choose("vaeSourcePath")} /></div>}</div><DialogActions busy={busy} ready={ready} onClose={onClose} onSubmit={() => void submit()} /></RepositoryDialog>;
}
