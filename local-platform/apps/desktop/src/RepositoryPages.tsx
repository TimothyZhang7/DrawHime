...
function RepositoryCard({ title, subtitle, description, coverPath, fallback, secondaryTag, onOpen, action, progress }: { title: string; subtitle: string; description: string; coverPath: string | null; fallback: string; secondaryTag: string; onOpen: () => void; action: { kind: "local" | "download" | "view"; label: string; disabled: boolean; loading?: boolean; onClick: () => void }; progress: number | null }) {
  // 卡片动作即使被禁用也要说明当前状态，避免下载和已安装状态只剩视觉颜色差异。
  ...
  return <article className="repository-card" tabIndex={0} role="button" onClick={onOpen} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") onOpen(); }}><RepositoryCover path={coverPath} fallback={fallback} compact /><div className="repository-card-tags"><i>{secondaryTag}</i></div><div className="repository-card-copy"><strong>{title}</strong><span>{subtitle}</span><p>{description}</p></div>{progress !== null && <div className="repository-card-progress"><i style={{ width: `${progress}%` }} /></div>}<button className={`is-${action.kind}`} disabled={action.disabled} title={actionTitle} onClick={(event) => { event.stopPropagation(); action.onClick(); }}>{action.loading ? <LoaderCircle className="spin" /> : action.kind === "local" ? <CheckCircle2 /> : action.kind === "download" ? <Download /> : <ChevronRight />}{action.label}</button></article>;
}
...
function RepositoryCover({ path, fallback, compact = false }: { path: string | null; fallback: string; compact?: boolean }) {
  ...
  return <div className={`repository-cover ${compact ? "is-compact" : ""} ${examples.length > 1 ? "has-examples" : ""}`} aria-label={path ? "仓库图片" : `${fallback} 默认封面`}><div className="repository-cover-fallback"><Image /><span>{fallback}</span></div>{path && <>{!compact && <img className="blur" decoding="async" src={convertFileSrc(path)} alt="" onError={hideBrokenImage} />}<div className="repository-cover-slides">{examples.map((example, index) => <img key={example} loading={compact || index > 0 ? "lazy" : "eager"} decoding="async" style={{ animationDelay: `${index * 2.8}s`, animationDuration: `${examples.length * 2.8}s` }} src={convertFileSrc(example)} alt="" onError={hideBrokenImage} />)}</div>{examples.length > 1 && <span className="repository-example-count">{examples.length} 张示例</span>}</>}</div>;
}
