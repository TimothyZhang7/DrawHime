...
function RepositoryCover({ path, fallback }: { path: string | null; fallback: string }) {
  ...
  return <div className={`repository-cover ${examples.length > 1 ? "has-examples" : ""}`}>{path ? <><img className="blur" decoding="async" src={convertFileSrc(path)} alt="" /><div className="repository-cover-slides">{examples.map((example, index) => <img key={example} loading={index === 0 ? "eager" : "lazy"} decoding="async" style={{ animationDelay: `${index * 2.8}s`, animationDuration: `${examples.length * 2.8}s` }} src={convertFileSrc(example)} alt={index === 0 ? "仓库封面" : `示例图 ${index + 1}`} />)}</div>{examples.length > 1 && <span className="repository-example-count">{examples.length} 张示例</span>}</> : <div className="repository-cover-fallback"><Image /><span>{fallback}</span></div>}</div>;
}
