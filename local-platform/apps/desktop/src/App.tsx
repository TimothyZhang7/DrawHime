...
import { Activity, AlertTriangle, BookOpenCheck, CheckCircle2, CircleUserRound, Cpu, Database, Download, FlaskConical, FolderCog, FolderPlus, Gauge, HardDrive, Image, Images, KeyRound, Layers3, LoaderCircle, MemoryStick, Monitor, Moon, PackageCheck, PackageOpen, Play, Power, RefreshCw, Save, Settings2, ShieldCheck, Sparkles, Sun, Tags, Upload, UploadCloud, X } from "lucide-react";
...
type DesktopPage = "overview" | "generate" | "training" | "models" | "loras" | "gallery" | "settings";
type OverviewSection = "overview" | "account" | "environment" | "resources";
type GallerySection = "jobs" | "sync";
type SettingsSection = "general" | "ai" | "updates";

const navigation = [
  { id: "overview" as const, label: "概览 / 账号", Icon: Gauge },
...
export function App() {
  ...
  const [overviewSection, setOverviewSection] = useState<OverviewSection>("overview");
  const [gallerySection, setGallerySection] = useState<GallerySection>("jobs");
  ...
  const downloadResource = async (resourceId: string) => {
    ...
    setResourceProgress((current) => ({ ...current, [resourceId]: { resourceId, status: "queued", sourceKind: null, downloadedBytes: 0, totalBytes, bytesPerSecond: 0, etaSeconds: null, switchReason: null, targetPath: null, error: null } }));
  ...
  return <div className="desktop-shell" data-environment-status={bootstrap.environment.status} data-inference-ready={bootstrap.environment.capabilities.inference} data-training-ready={bootstrap.environment.capabilities.training}>
    ...
    <main className="desktop-main">
      ...
      {bootstrap.environment.status !== "ready" && critical && <section className={`environment-banner is-${critical.severity}`}><AlertTriangle /><div><strong>{critical.title}</strong><span>{critical.message}</span></div><button onClick={() => { setPage("overview"); setOverviewSection("environment"); }}>查看环境详情</button></section>}
      ...
      <div hidden={page !== "training"}><TrainingPage datasets={trainingDatasets} captionJobs={captionJobs} trainingJobs={trainingJobs} models={models} captioningReady={bootstrap.environment.capabilities.captioning} trainingReady={bootstrap.environment.capabilities.training} onUpdated={trainingDatasetUpdated} onCaptionJobUpdated={captionJobUpdated} onTrainingJobUpdated={trainingJobUpdated} onOpenResources={() => { setPage("overview"); setOverviewSection("resources"); }} onError={setMessage} /></div>
      <div hidden={page !== "overview"} className="workspace-page"><WorkspaceTabs label="概览与设备" value={overviewSection} onChange={(value) => setOverviewSection(value as OverviewSection)} items={[{ id: "overview", label: "设备概览", status: bootstrap.environment.status === "ready" ? "正常" : "待处理" }, { id: "account", label: "账号", status: account.status === "connected" ? "已连接" : "未连接" }, { id: "environment", label: "环境检测", status: `${bootstrap.environment.issues.length} 项` }, { id: "resources", label: "资源下载", status: `${resourceCatalog?.resources.filter((item) => !item.installed).length || 0} 待安装` }]} /><div hidden={overviewSection !== "overview"}><OverviewPage state={bootstrap} runtimeBusy={runtimeBusy} onRuntimeAction={(action) => void controlRuntime(action)} /></div><div hidden={overviewSection !== "account"}><AccountPage account={account} onChanged={(next) => { setAccount(next); if (next.status === "connected") void Promise.all([loadDesktopWebsiteModels(), loadDesktopWebsiteLoras()]).then(([remoteModels, remoteLoras]) => { setWebsiteModels(remoteModels); setWebsiteLoras(remoteLoras); }).catch((error) => setMessage(errorMessage(error))); else { setWebsiteModels([]); setWebsiteLoras([]); } }} onError={setMessage} /></div><div hidden={overviewSection !== "environment"}><EnvironmentPage report={bootstrap.environment} /></div><div hidden={overviewSection !== "resources"}><ResourcesPage catalog={resourceCatalog} progress={resourceProgress} installProgress={installProgress} loading={catalogLoading} bulkBusy={resourceBulkBusy} onReload={() => void reloadResourceCatalog()} onInstallRequired={() => void installRequiredResources()} onDownload={(resourceId) => void downloadResource(resourceId)} onPause={(resourceId) => void pauseDesktopResourceDownload(resourceId)} onInstall={(resourceId) => void installResource(resourceId)} /></div></div>
      <div hidden={page !== "gallery"} className="workspace-page"><WorkspaceTabs label="本地图库" value={gallerySection} onChange={(value) => setGallerySection(value as GallerySection)} items={[{ id: "jobs", label: "生成记录", status: `${jobs.length} 项` }, { id: "sync", label: "同步队列", status: `${queue.filter((item) => item.status !== "synced").length} 待处理` }]} /><div hidden={gallerySection !== "jobs"}><JobsPage jobs={jobs} onCancel={(id) => void cancelJob(id)} /></div><div hidden={gallerySection !== "sync"}><SyncPage items={queue} /></div></div>
      <div hidden={page !== "settings"} className="workspace-page"><WorkspaceTabs label="应用设置" value={settingsSection} onChange={(value) => setSettingsSection(value as SettingsSection)} items={[{ id: "general", label: "基础设置" }, { id: "ai", label: "AI 辅助" }, { id: "updates", label: "软件更新", status: softwareUpdateStatusLabel(softwareUpdate?.status || "unavailable") }]} /><div hidden={settingsSection !== "general"}><SettingsPage value={bootstrap.settings} onSaved={(settings) => { setBootstrap((current) => current ? { ...current, settings } : current); setMessage("本地设置已保存"); void reloadResourceCatalog(); }} onError={setMessage} /></div><div hidden={settingsSection !== "ai"}><AiSettingsCard onMessage={setMessage} /></div><div hidden={settingsSection !== "updates"}><UpdatesPage value={softwareUpdate} onChanged={setSoftwareUpdate} onError={setMessage} /></div></div>
    </main>
  </div>;
}
...
function GeneratePage({ models, loras, inferenceReady, defaultPrivacy, onCreated, onError }: { models: DesktopLocalModelView[]; loras: DesktopLocalLoraView[]; inferenceReady: boolean; defaultPrivacy: DesktopSettings["defaultPrivacy"]; onCreated: (job: DesktopLocalJobView) => void; onError: (message: string) => void }) {
...
function UpdatesPage({ value, onChanged, onError }: { value: DesktopSoftwareUpdateView | null; onChanged: (value: DesktopSoftwareUpdateView) => void; onError: (message: string) => void }) {
  ...
  const apply = () => { if (window.confirm("应用更新后程序将退出，安装器会静默完成替换。是否继续？")) void execute("apply", applyDesktopSoftwareUpdate); };
...
function pageTitle(page: DesktopPage): string { return { overview: "概览 / 账号", generate: "本地生成", training: "LoRA 训练", models: "模型仓库", loras: "LoRA 仓库", gallery: "图库", settings: "设置" }[page]; }
...
function resourceKindLabel(kind: string): string { return { runtime: "运行环境", model: "底模", lora: "LoRA", captioner: "打标模型", trainer: "训练组件", application: "软件更新" }[kind] || kind; }
function sourceKindLabel(kind: string): string { return { official: "官方", mirror: "主站镜像" }[kind] || kind; }
function downloadStatusLabel(status: DesktopResourceDownloadView["status"]): string { return { queued: "排队中", downloading: "下载中", paused: "已暂停", verifying: "校验中", downloaded: "已完成", failed: "失败" }[status]; }
...
function formatEta(seconds: number): string { if (seconds < 60) return `${seconds} 秒`; if (seconds < 3600) return `${Math.ceil(seconds / 60)} 分钟`; return `${Math.floor(seconds / 3600)} 小时 ${Math.ceil(seconds % 3600 / 60)} 分钟`; }
