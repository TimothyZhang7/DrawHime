...
export function App() {
  ...
  return <div className="desktop-shell" data-environment-status={bootstrap.environment.status} data-inference-ready={bootstrap.environment.capabilities.inference} data-training-ready={bootstrap.environment.capabilities.training}>
    ...
    <main className="desktop-main">
      ...
      {bootstrap.environment.status !== "ready" && critical && <section className={`environment-banner is-${critical.severity}`}><AlertTriangle /><div><strong>{critical.title}</strong><span>{critical.message}</span></div><button onClick={() => setPage("overview")}>查看环境详情</button></section>}
      ...
      <div hidden={page !== "training"}><TrainingPage datasets={trainingDatasets} captionJobs={captionJobs} trainingJobs={trainingJobs} models={models} captioningReady={bootstrap.environment.capabilities.captioning} trainingReady={bootstrap.environment.capabilities.training} onUpdated={trainingDatasetUpdated} onCaptionJobUpdated={captionJobUpdated} onTrainingJobUpdated={trainingJobUpdated} onOpenResources={() => setPage("overview")} onError={setMessage} /></div>
      <div hidden={page !== "overview"} className="merged-page"><OverviewPage state={bootstrap} runtimeBusy={runtimeBusy} onRuntimeAction={(action) => void controlRuntime(action)} /><AccountPage account={account} onChanged={(next) => { setAccount(next); if (next.status === "connected") void Promise.all([loadDesktopWebsiteModels(), loadDesktopWebsiteLoras()]).then(([remoteModels, remoteLoras]) => { setWebsiteModels(remoteModels); setWebsiteLoras(remoteLoras); }).catch((error) => setMessage(errorMessage(error))); else { setWebsiteModels([]); setWebsiteLoras([]); } }} onError={setMessage} /><EnvironmentPage report={bootstrap.environment} /><ResourcesPage catalog={resourceCatalog} progress={resourceProgress} installProgress={installProgress} loading={catalogLoading} bulkBusy={resourceBulkBusy} onReload={() => void reloadResourceCatalog()} onInstallRequired={() => void installRequiredResources()} onDownload={(resourceId) => void downloadResource(resourceId)} onPause={(resourceId) => void pauseDesktopResourceDownload(resourceId)} onInstall={(resourceId) => void installResource(resourceId)} /></div>
      <div hidden={page !== "gallery"} className="merged-page"><JobsPage jobs={jobs} onCancel={(id) => void cancelJob(id)} /><SyncPage items={queue} /></div>
      <div hidden={page !== "settings"} className="merged-page"><SettingsPage value={bootstrap.settings} onSaved={(settings) => { setBootstrap((current) => current ? { ...current, settings } : current); setMessage("本地设置已保存"); void reloadResourceCatalog(); }} onError={setMessage} /><AiSettingsCard onMessage={setMessage} /><UpdatesPage value={softwareUpdate} onChanged={setSoftwareUpdate} onError={setMessage} /></div>
    </main>
  </div>;
}
