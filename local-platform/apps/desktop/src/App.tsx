...
function SettingsPage({ value, onSaved, onError }: { value: DesktopSettings; onSaved: (settings: DesktopSettings) => void; onError: (message: string) => void }) {
  ...
  const save = async () => { setBusy(true); try { onSaved(await saveDesktopSettings({ ...form, dependencySource: "mirror" })); } catch (error) { onError(errorMessage(error)); } finally { setBusy(false); } };
