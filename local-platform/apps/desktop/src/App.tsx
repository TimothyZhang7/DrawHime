...
import { AlertTriangle, CheckCircle2, Cpu, Database, FolderCog, Gauge, HardDrive, Image, LoaderCircle, MemoryStick, Monitor, Moon, RefreshCw, Settings2, ShieldCheck, Sun, UploadCloud } from "lucide-react";
...
export function App() {
  ...
  useEffect(() => { void loadDesktopBootstrap().then(async (state) => { lastEnvironmentCheckAt.current = Date.now(); setBootstrap(state); setQueue(await listDesktopGallerySyncQueue()); }).catch((error) => setMessage(errorMessage(error))).finally(() => setLoading(false)); }, []);
  useEffect(() => {
