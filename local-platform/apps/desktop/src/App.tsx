...
export function App() {
  ...
  return <div className="desktop-shell" data-environment-status={bootstrap.environment.status} data-inference-ready={bootstrap.environment.capabilities.inference} data-training-ready={bootstrap.environment.capabilities.training}>
    ...
    <main className="desktop-main">
      ...
      <div hidden={page !== "generate"}><GenerationPage models={models} loras={loras} inferenceReady={bootstrap.environment.capabilities.inference} defaultPrivacy={bootstrap.settings.defaultPrivacy} onCreated={jobCreated} onError={setMessage} /></div>
      ...
      <div hidden={page !== "training"}><TrainingPage datasets={trainingDatasets} captionJobs={captionJobs} trainingJobs={trainingJobs} models={models} captioningReady={bootstrap.environment.capabilities.captioning} trainingReady={bootstrap.environment.capabilities.training} onUpdated={trainingDatasetUpdated} onCaptionJobUpdated={captionJobUpdated} onTrainingJobUpdated={trainingJobUpdated} onOpenResources={() => { setPage("overview"); setOverviewSection("start"); }} onError={setMessage} /></div>
       <div hidden={page !== "overview"} className="workspace-page"><WorkspaceTabs label="启动与账号" value={overviewSection} onChange={(value) => setOverviewSection(value as OverviewSection)} items={[{ id: "start", label: "启动", status: bootstrap.runtime.status === "ready" ? "运行中" : startupInitialized ? "可启动" : "待初始化" }, { id: "account", label: "账号", status: account.status === "connected" ? "已连接" : "未连接" }]} /><div hidden={overviewSection !== "start"}><StartupPage state={bootstrap} catalog={resourceCatalog} progress={resourceProgress} installProgress={installProgress} phase={startupPhase} checking={checking} onPrimary={() => void runStartup()} onRecheck={() => void recheck()} onOpenQueue={() => setDownloadQueueOpen(true)} /></div><div hidden={overviewSection !== "account"}><AccountPage account={account} onChanged={(next) => { setAccount(next); if (next.status === "connected") void Promise.all([loadDesktopWebsiteModels(), loadDesktopWebsiteLoras()]).then(([remoteModels, remoteLoras]) => { setWebsiteModels(remoteModels); setWebsiteLoras(remoteLoras); }).catch((error) => setMessage(errorMessage(error))); else { setWebsiteModels([]); setWebsiteLoras([]); } }} onError={setMessage} /></div></div>
      <div hidden={page !== "gallery"} className="workspace-page"><WorkspaceTabs label="图库与本地记录" value={gallerySection} onChange={(value) => setGallerySection(value as GallerySection)} items={[{ id: "gallery", label: "图库", status: `${jobs.filter((job) => job.artifact).length} 张` }, { id: "jobs", label: "记录", status: `${jobs.length} 项` }]} /><div hidden={gallerySection !== "gallery"}><LocalGalleryPage jobs={jobs} loras={loras} websiteLoras={websiteLoras} /></div><div hidden={gallerySection !== "jobs"}><LocalJobsPage jobs={jobs} loras={loras} websiteLoras={websiteLoras} onCancel={(id) => void cancelJob(id)} /></div></div>
...
function pageTitle(page: DesktopPage): string { return { overview: "启动 / 账号", generate: "本地生成", training: "LoRA 训练", models: "模型仓库", loras: "LoRA 仓库", gallery: "图库 / 记录", settings: "设置" }[page]; }
