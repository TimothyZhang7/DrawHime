...
use crate::models::{
    DesktopResourceCatalogItemView, DesktopResourceCatalogView, DesktopResourceDownloadView,
    DesktopResourceInstallView, DesktopResourceManifestEnvelope, DesktopResourceManifestItem,
    DesktopResourceManifestPayload, DesktopResourceSource, DesktopSettings,
};
...
pub fn load_catalog(
    ...
    let (payload, cached_manifest) =
        load_resource_manifest(manifest_url, key_id, public_key, app_data_dir)?;
    ...
    let resources = payload
        ...
        .map(|item| {
            ...
            DesktopResourceCatalogItemView {
                ...
                installed,
                install_path: installed.then(|| {
                    install_destination(item, settings)
                        .to_string_lossy()
                        .into_owned()
                }),
                source_kinds: mirror_sources(item)
                    .iter()
        ...
        .collect();
    let message = if cached_manifest {
        "网络暂不可用，已使用仍在有效期内的本机签名清单"
    } else {
        "资源清单签名和有效期校验通过"
    };
    Ok(DesktopResourceCatalogView {
        configured: true,
...
pub fn install_resource(
    ...
    resource_id: &str,
    app: &tauri::AppHandle,
) -> Result<ResourceInstallOutcome, String> {
    let result = install_resource_inner(settings, app_data_dir, resource_id, app);
    if let Err(error) = &result {
        emit_install_progress(
            app,
            install_view(resource_id, "failed", 0, None, None, Some(error.clone())),
        );
    }
    result
}

fn install_resource_inner(
    settings: &DesktopSettings,
    app_data_dir: &Path,
    resource_id: &str,
    app: &tauri::AppHandle,
    ...
    if item.kind == "application" {
        return Err("应用程序包请在软件更新页面应用".into());
    }
    ...
    let view = match install_cached_resource(settings, &item, &cache, &notify) {
        ...
        Err(error) if archive_cache_should_recover(&item, &error) => {
            ...
            download_resource(settings, app_data_dir, resource_id, app).map_err(
                |download_error| format!("资源归档缓存异常，自动重新下载失败：{download_error}"),
            )?;
            install_cached_resource(settings, &item, &cache, &notify).map_err(|retry_error| {
                format!("资源归档重新下载并校验后仍不可读取：{retry_error}")
            })?
        }
        Err(error) => return Err(error),
    };
    let model_registrations = collect_model_registrations(settings, &payload.resources)?;
    // 安装目录和标记已经独立校验；释放下载副本可避免 Runtime、模型和缓存长期各占一份空间。
    if let Err(error) = release_installed_cache(&cache) {
        eprintln!("{error}");
    }
    Ok(ResourceInstallOutcome {
        view,
        model_registrations,
    })
}

fn release_installed_cache(cache: &Path) -> Result<(), String> {
    for path in [cache.to_path_buf(), marker_path(cache), partial_path(cache)] {
        if path.is_file() {
            fs::remove_file(&path).map_err(|error| format!("释放已安装资源缓存失败：{error}"))?;
        }
    }
    Ok(())
}

/** 返回当前签名清单中已经安装完成、可安全释放的依赖缓存路径。 */
pub(crate) fn reclaimable_installed_cache_paths(
    settings: &DesktopSettings,
    app_data_dir: &Path,
) -> Vec<PathBuf> {
    let Some((manifest_url, key_id, public_key)) = manifest_configuration() else {
    ...
    let Ok((payload, _)) = load_resource_manifest(manifest_url, key_id, public_key, app_data_dir)
    else {
        return Vec::new();
    };
    let cache_root = resource_cache_dir(app_data_dir);
    payload
        .resources
        .iter()
        .filter(|item| item.kind != "application" && installed_resource_matches(item, settings))
        .flat_map(|item| {
            let cache = cache_root.join(&item.file_name);
            [cache.clone(), marker_path(&cache), partial_path(&cache)]
        })
        .filter(|path| path.is_file())
        .collect()
}

fn install_cached_resource<F: Fn(DesktopResourceInstallView)>(
    settings: &DesktopSettings,
    ...
    if installed_resource_matches(&item, settings) {
        return Ok(install_view(
            &item.id,
            "installed",
            100,
            Some(&destination),
    ...
    let parent = destination
        .parent()
        .ok_or_else(|| "资源安装目录缺少父路径".to_string())?;
    ...
    let available =
        available_space(parent).map_err(|error| format!("读取安装磁盘空间失败：{error}"))?;
    ...
    let install_candidate = if item.archive != "raw" {
        ...
        let extraction = if item.archive == "zip" {
            extract_zip_safely(cache, &content, item.installed_size)
        } else {
            extract_7z_safely(cache, &content, item.installed_size)
        };
        ...
        let root = item
            .root_directory
            .as_deref()
            .map(|directory| content.join(directory))
            .unwrap_or(content);
        if !root.is_dir() {
            return Err("资源归档声明的根目录不存在".into());
        }
        validate_extracted_resource(item, &root)?;
        if item.kind == "runtime" {
            write_runtime_manifest(&root, item)?;
        }
        write_install_marker(&root, item, true)?;
        root
    } else {
        if fs::hard_link(&cache, &staging).is_err() {
            fs::copy(&cache, &staging)
                .map_err(|error| format!("复制资源到安装临时文件失败：{error}"))?;
        }
        staging.clone()
    };
    notify(install_view(
    ...
    if item.archive != "raw" && staging.exists() {
        let _ = fs::remove_dir_all(&staging);
    }
...
pub fn download_resource(
    ...
    let source = item
        .sources
        .first()
        .ok_or_else(|| "签名清单没有可用的主站镜像地址".to_string())?;
    let cache_dir = resource_cache_dir(app_data_dir);
    ...
    if partial
        .metadata()
        .map(|metadata| metadata.len() > item.byte_size)
        .unwrap_or(false)
    {
        fs::remove_file(&partial).map_err(|error| format!("清理超长下载断点失败：{error}"))?;
    }
    let session_start_bytes = partial
        .metadata()
        .map(|metadata| metadata.len())
        .unwrap_or(0);
    emit_progress(
        app,
        progress_view(&item, "queued", None, session_start_bytes, 0, None, None),
    );
    ...
    if let Err(error) = download_from_source(
        &client,
        &item,
        source,
        &partial,
...
pub fn pause_download(resource_id: &str) -> Result<(), String> {
    if resource_id.trim().is_empty() || resource_id.len() > 128 {
        return Err("资源 ID 不正确".into());
    }
    paused_downloads()
        .lock()
...
fn manifest_configuration() -> Option<(&'static str, &'static str, &'static str)> {
    (!MANIFEST_URL.is_empty() && !MANIFEST_KEY_ID.is_empty() && !MANIFEST_PUBLIC_KEY.is_empty())
        .then_some((MANIFEST_URL, MANIFEST_KEY_ID, MANIFEST_PUBLIC_KEY))
}

/** 为软件更新控制器读取同一固定公钥验签后的在线清单。 */
pub(crate) fn verified_manifest() -> Result<DesktopResourceManifestPayload, String> {
    let Some((manifest_url, key_id, public_key)) = manifest_configuration() else {
        return Err("当前安装包尚未配置经过签名的更新通道".into());
    };
...
pub(crate) fn cached_resource_path(
    ...
    resource_cache_dir(app_data_dir).join(&item.file_name)
}

/** 导入离线 NSIS 包及其 Ed25519 信封，验证完成后写入与在线下载相同的受控缓存。 */
pub(crate) fn import_offline_application(
    app_data_dir: &Path,
    installer_path: &Path,
    envelope_path: &Path,
) -> Result<DesktopResourceManifestItem, String> {
    if !installer_path.is_file() || !envelope_path.is_file() {
        return Err("离线更新安装包或签名信封不存在".into());
    }
    ...
    if target.exists() {
        quarantine_file(&target, "replaced")?;
    }
    if target_envelope.exists() {
        quarantine_file(&target_envelope, "replaced")?;
    }
    fs::rename(&temporary, &target).map_err(|error| format!("提交离线更新缓存失败：{error}"))?;
    fs::rename(&temporary_envelope, &target_envelope)
        .map_err(|error| format!("提交离线更新签名失败：{error}"))?;
    write_verified_marker(&target, &item)?;
    Ok(item)
}

/** 重新验证离线包签名与完整性，应用更新前不得只信任 SQLite 或验证标记。 */
pub(crate) fn verify_offline_application(
    installer_path: &Path,
    envelope_path: &Path,
) -> Result<DesktopResourceManifestItem, String> {
    verify_application_package(
        installer_path,
        envelope_path,
        MANIFEST_KEY_ID,
        MANIFEST_PUBLIC_KEY,
...
fn verify_application_package(
    ...
    let envelope_bytes =
        fs::read(envelope_path).map_err(|error| format!("读取离线更新签名失败：{error}"))?;
    if envelope_bytes.len() as u64 > MAX_MANIFEST_BYTES {
        return Err("离线更新签名信封超过大小限制".into());
    }
    let envelope: DesktopResourceManifestEnvelope = serde_json::from_slice(&envelope_bytes)
        .map_err(|error| format!("解析离线更新签名失败：{error}"))?;
    let payload = verify_manifest(envelope, expected_key_id, public_key)?;
    let file_name = installer_path
        .file_name()
        .and_then(|value| value.to_str())
        .ok_or_else(|| "离线更新文件名不正确".to_string())?;
    let item = payload
        .resources
        .into_iter()
        .find(|item| {
    ...
    if !file_matches(installer_path, &item)? {
        return Err("离线更新安装包大小或 SHA-256 与签名不一致".into());
    }
    Ok(item)
}

/** 离线签名信封与安装包同目录保存，回滚和应用时都重新验签。 */
pub(crate) fn offline_envelope_path(installer_path: &Path) -> PathBuf {
    installer_path.with_file_name(format!(
        "{}.envelope.json",
        installer_path
            .file_name()
            .unwrap_or_default()
...
fn fetch_verified_manifest(
    ...
    expected_key_id: &str,
    public_key: &str,
) -> Result<DesktopResourceManifestPayload, String> {
    verify_manifest(
        fetch_manifest_envelope(manifest_url)?,
        expected_key_id,
        public_key,
    )
}
...
fn load_resource_manifest(
    ...
    public_key: &str,
    app_data_dir: &Path,
) -> Result<(DesktopResourceManifestPayload, bool), String> {
    match fetch_manifest_envelope(manifest_url) {
        ...
        Err(online_error) => {
            let cached = read_cached_resource_manifest(app_data_dir)
                .and_then(|envelope| verify_manifest(envelope, expected_key_id, public_key));
            cached
                .map(|payload| (payload, true))
                .map_err(|cached_error| {
                    format!("{online_error}；本机签名清单不可用：{cached_error}")
                })
        }
    }
}
...
fn persist_cached_resource_manifest(
    app_data_dir: &Path,
    envelope: &DesktopResourceManifestEnvelope,
) -> Result<(), String> {
    ...
    let bytes =
        serde_json::to_vec(envelope).map_err(|error| format!("序列化资源签名清单失败：{error}"))?;
    fs::write(&temporary, bytes)
        .map_err(|error| format!("写入资源签名清单临时文件失败：{error}"))?;
    if backup.exists() {
        fs::remove_file(&backup).map_err(|error| format!("清理旧资源清单回滚文件失败：{error}"))?;
    }
    if target.exists() {
        fs::rename(&target, &backup).map_err(|error| format!("备份旧资源签名清单失败：{error}"))?;
    }
...
fn read_cached_resource_manifest(
    app_data_dir: &Path,
) -> Result<DesktopResourceManifestEnvelope, String> {
    let path = cached_resource_manifest_path(app_data_dir);
    let metadata = path
        .metadata()
        .map_err(|_| "尚无本机签名清单缓存".to_string())?;
    if !metadata.is_file() || metadata.len() > MAX_MANIFEST_BYTES {
        return Err("本机签名清单缓存大小异常".into());
    }
...
fn fetch_manifest_envelope(manifest_url: &str) -> Result<DesktopResourceManifestEnvelope, String> {
    ...
    if url.scheme() != "https" {
        return Err("资源清单必须使用 HTTPS".into());
    }
    let client = online_client_builder()
        .connect_timeout(Duration::from_secs(5))
        .timeout(Duration::from_secs(12))
        .build()
        .map_err(|error| format!("创建清单客户端失败：{error}"))?;
    let response = client
        .get(url)
        .send()
        .map_err(|error| format!("获取资源清单失败：{}", network_error(&error)))?;
    if !response.status().is_success() {
        return Err(format!("资源清单返回 HTTP {}", response.status().as_u16()));
    }
    if response
        .content_length()
        .is_some_and(|length| length > MAX_MANIFEST_BYTES)
    {
        return Err("资源清单超过大小限制".into());
    }
    ...
    let wrapper: ManifestApiResponse =
        serde_json::from_slice(&bytes).map_err(|error| format!("解析资源清单响应失败：{error}"))?;
    if !wrapper.ok {
        return Err(wrapper
            .message
            .unwrap_or_else(|| "资源服务返回失败状态".into()));
    }
    wrapper
        .data
        .ok_or_else(|| "资源服务未返回签名清单".to_string())
}

/** 保存当前在线清单的原始签名信封，使已下载版本在后续回滚时仍可重新验签。 */
pub(crate) fn persist_online_application_envelope(
    app_data_dir: &Path,
    item: &DesktopResourceManifestItem,
    ...
    if !payload.resources.iter().any(|candidate| {
        candidate.id == item.id
            && candidate.file_name == item.file_name
            && candidate.sha256 == item.sha256
            && candidate.byte_size == item.byte_size
            && candidate.version == item.version
    }) {
        return Err("在线签名清单已切换到其他更新版本".into());
    }
    ...
    fs::write(
        &temporary,
        serde_json::to_vec(&envelope).map_err(|error| format!("序列化更新签名失败：{error}"))?,
    )
    .map_err(|error| format!("保存在线更新签名失败：{error}"))?;
    if target.exists() {
        quarantine_file(&target, "replaced")?;
    }
...
fn verify_manifest(
    ...
    if envelope.key_id != expected_key_id {
        return Err("资源清单签名密钥标识不匹配".into());
    }
    let key_bytes = BASE64
        .decode(public_key)
        .map_err(|_| "资源公钥编码不正确".to_string())?;
    let key_array: [u8; 32] = key_bytes
        .try_into()
        .map_err(|_| "资源公钥长度不正确".to_string())?;
    let verifying_key =
        VerifyingKey::from_bytes(&key_array).map_err(|_| "资源公钥内容不正确".to_string())?;
    let signature_bytes = BASE64
        .decode(&envelope.signature)
        .map_err(|_| "资源清单签名编码不正确".to_string())?;
    let signature = Signature::from_slice(&signature_bytes)
        .map_err(|_| "资源清单签名长度不正确".to_string())?;
    verifying_key
        .verify(envelope.payload.as_bytes(), &signature)
        .map_err(|_| "资源清单签名验证失败".to_string())?;
    let payload: DesktopResourceManifestPayload = serde_json::from_str(&envelope.payload)
        .map_err(|error| format!("解析已签名资源载荷失败：{error}"))?;
...
fn validate_manifest(payload: &DesktopResourceManifestPayload) -> Result<(), String> {
    if payload.schema_version != 1 {
        return Err("资源清单版本不受支持".into());
    }
    if !matches!(payload.channel.as_str(), "stable" | "beta") {
        return Err("资源清单发布通道不正确".into());
    }
    let generated_at = DateTime::parse_from_rfc3339(&payload.generated_at)
        .map_err(|_| "资源清单生成时间不正确".to_string())?
        .with_timezone(&Utc);
    let expires_at = DateTime::parse_from_rfc3339(&payload.expires_at)
        .map_err(|_| "资源清单过期时间不正确".to_string())?
        .with_timezone(&Utc);
    if generated_at > Utc::now() + ChronoDuration::minutes(5) {
        return Err("资源清单生成时间晚于本机时间".into());
    }
    if expires_at <= Utc::now() || expires_at <= generated_at {
        return Err("资源清单已经过期".into());
    }
    if payload.resources.len() > 500 {
        return Err("资源清单项目过多".into());
    }
    let mut ids = HashSet::new();
    for item in &payload.resources {
        if !ids.insert(&item.id) {
            return Err(format!("资源清单存在重复 ID：{}", item.id));
        }
        validate_item(item)?;
    }
...
fn validate_model_groups(items: &[DesktopResourceManifestItem]) -> Result<(), String> {
    ...
    for item in items {
        let Some(registration) = &item.model_registration else {
            continue;
        };
        if !matches!(registration.workflow_kind.as_str(), "checkpoint" | "anima")
            || !matches!(
                registration.role.as_str(),
                "primary" | "text_encoder" | "vae"
            )
        {
            return Err(format!("模型组合字段不正确：{}", item.id));
        }
        let group = groups.entry(&registration.group_id).or_insert((
            &registration.display_name,
            &registration.family,
            &registration.workflow_kind,
            HashSet::new(),
        ));
        if group.0 != registration.display_name
            || group.1 != registration.family
            || group.2 != registration.workflow_kind
            || !group.3.insert(&registration.role)
        {
            return Err(format!("模型组合元数据冲突：{}", registration.group_id));
        }
    }
    for (group_id, (_, _, workflow_kind, roles)) in groups {
        let valid = if workflow_kind == "anima" {
            roles.len() == 3
                && ["primary", "text_encoder", "vae"]
                    .iter()
                    .all(|role| roles.contains(role))
        } else {
            roles.len() == 1 && roles.contains("primary")
        };
        if !valid {
            return Err(format!("模型组合资源不完整：{group_id}"));
        }
    }
    Ok(())
}
...
fn validate_item(item: &DesktopResourceManifestItem) -> Result<(), String> {
    ...
    if item.file_name.len() < 2
        || item.file_name.len() > 255
        || !item.file_name.chars().all(|character| {
            character.is_ascii_alphanumeric() || matches!(character, '.' | '_' | '-')
        })
    {
        return Err(format!("资源文件名不安全：{}", item.id));
    }
    validate_windows_relative_path(Path::new(&item.file_name))
        .map_err(|_| format!("资源文件名不适用于 Windows：{}", item.id))?;
    if item.byte_size == 0
        || item.installed_size == 0
        || item.installed_size > 512 * 1024 * 1024 * 1024
        || item.sha256.len() != 64
        || !item
            .sha256
            .chars()
    ...
    {
        return Err(format!("资源大小或哈希不正确：{}", item.id));
    }
    if !matches!(item.archive.as_str(), "raw" | "zip" | "7z")
        || item.sources.is_empty()
        || item.sources.len() > 8
    {
        return Err(format!("资源归档或来源不正确：{}", item.id));
    }
    if matches!(item.kind.as_str(), "model" | "lora")
        && (item.archive != "raw" || item.installed_size != item.byte_size)
    {
        return Err(format!(
            "模型和 LoRA 必须使用声明大小一致的原始文件：{}",
            item.id
        ));
    }
    ...
    if item.kind == "application"
        && (item.archive != "raw"
            || item.application_update.is_none()
            || !item.file_name.to_ascii_lowercase().ends_with(".exe"))
    {
        return Err(format!(
            "应用更新必须是带版本元数据的原始 NSIS EXE：{}",
            item.id
        ));
    }
    if let Some(metadata) = &item.application_update {
        let version = semantic_version(&item.version)
            .ok_or_else(|| format!("应用更新版本格式不正确：{}", item.id))?;
        let minimum = semantic_version(&metadata.minimum_version)
            .ok_or_else(|| format!("应用更新最低版本格式不正确：{}", item.id))?;
        if minimum > version || metadata.release_notes.len() > 20_000 {
            return Err(format!("应用更新版本门禁或说明不正确：{}", item.id));
        }
    }
    if item.kind != "application" && item.application_update.is_some() {
        return Err(format!("非应用资源不得声明软件更新元数据：{}", item.id));
    }
    if item.archive == "raw" && item.root_directory.is_some() {
        return Err(format!("原始文件资源不得声明归档根目录：{}", item.id));
    }
    if item.kind == "model"
        && (!matches!(
            item.install_directory.as_deref(),
            Some("checkpoints" | "diffusion_models" | "text_encoders" | "vae")
        ) || item.model_registration.is_none())
    {
        return Err(format!("模型资源缺少受控安装目录或组合登记：{}", item.id));
    }
    if item.kind == "lora"
        && item
            .install_directory
            .as_deref()
            .is_some_and(|directory| directory != "loras")
    {
        return Err(format!("LoRA 安装目录不正确：{}", item.id));
    }
    if !matches!(item.kind.as_str(), "model" | "lora")
        && (item.install_directory.is_some() || item.model_registration.is_some())
    {
        return Err(format!("非模型资源不得声明模型安装元数据：{}", item.id));
    }
    if let Some(root_directory) = &item.root_directory {
        validate_windows_relative_path(Path::new(root_directory))
            .map_err(|_| format!("资源归档根目录不安全：{}", item.id))?;
        if Path::new(root_directory).components().count() != 1 {
            return Err(format!("资源归档根目录只能包含一级：{}", item.id));
        }
    }
    if item.sources.len() != 1 {
        return Err(format!("资源必须且只能声明一个主站镜像：{}", item.id));
    }
    let mut urls = HashSet::new();
    for source in &item.sources {
        if source.kind != "mirror" || !urls.insert(&source.url) {
            return Err(format!("资源来源类型或地址不正确、重复：{}", item.id));
        }
        let url =
            Url::parse(&source.url).map_err(|_| format!("资源下载地址格式不正确：{}", item.id))?;
        if url.scheme() != "https" || url.host_str() != Some("www.xanime.ink") {
            return Err(format!("资源下载地址必须使用绘图姬主站 HTTPS：{}", item.id));
        }
    }
    Ok(())
}
...
fn download_from_source<F: Fn(DesktopResourceDownloadView)>(
    client: &Client,
    item: &DesktopResourceManifestItem,
    source: &DesktopResourceSource,
    partial: &Path,
    session_started_at: Instant,
    session_start_bytes: u64,
    notify: &F,
) -> Result<(), String> {
    download_from_source_with_policy(
        client,
        item,
        source,
        partial,
...
fn download_from_source_with_policy<F: Fn(DesktopResourceDownloadView)>(
    client: &Client,
    item: &DesktopResourceManifestItem,
    source: &DesktopResourceSource,
    partial: &Path,
    ...
    let mut file = OpenOptions::new()
        ...
        .write(true)
        .open(partial)
        .map_err(|error| format!("打开下载断点失败：{error}"))?;
    file.seek(SeekFrom::Start(offset))
        .map_err(|error| format!("定位下载断点失败：{error}"))?;
...
fn validate_range_response(
    ...
    if content_length != Some(expected_length) {
        return Err("下载来源返回的分片长度不正确".into());
    }
    if !full_single_response {
        ...
        if content_range != Some(expected_range.as_str()) {
            return Err("下载来源返回的 Content-Range 不正确".into());
        }
    }
    Ok(())
}

fn stream_response<F: Fn(DesktopResourceDownloadView)>(
    ...
    loop {
        if download_paused(&item.id) {
            file.flush()
                .map_err(|error| format!("保存暂停断点失败：{error}"))?;
            return Err(DOWNLOAD_PAUSED_ERROR.into());
        }
        let read = response
            .read(&mut buffer)
            .map_err(|error| format!("读取中断：{error}"))?;
        if read == 0 {
            break;
        }
        let next = downloaded.saturating_add(read as u64);
        if next > expected_end {
            return Err("上游返回分片超过请求范围".into());
        }
        file.write_all(&buffer[..read])
            .map_err(|error| format!("写入下载断点失败：{error}"))?;
        downloaded = next;
        if last_emit.elapsed() >= Duration::from_millis(250) {
            notify(progress_view(
                item,
                "downloading",
                Some(source.kind.clone()),
                downloaded,
                session_average_speed(downloaded, session_start_bytes, session_started_at),
                None,
                None,
            ));
        ...
        if source_elapsed >= policy.low_speed_window && downloaded < item.byte_size {
            let source_speed =
                downloaded.saturating_sub(source_start_bytes) / source_elapsed.as_secs().max(1);
            if source_speed < policy.minimum_bytes_per_second {
                file.flush()
                    .map_err(|error| format!("保存低速断点失败：{error}"))?;
...
fn download_paused(resource_id: &str) -> bool {
    paused_downloads()
        .lock()
        .unwrap_or_else(|poisoned| poisoned.into_inner())
        .contains(resource_id)
}
fn clear_download_pause(resource_id: &str) {
    paused_downloads()
        .lock()
        .unwrap_or_else(|poisoned| poisoned.into_inner())
        .remove(resource_id);
...
/** 下载进度固定展示唯一的主站镜像。 */
fn source_kind_name(_kind: &str) -> &'static str {
    "主站镜像"
}
...
fn cached_resource_manifest_path(app_data_dir: &Path) -> PathBuf {
    resource_cache_dir(app_data_dir).join("manifest-envelope.json")
}
...
fn verified_marker_matches(target: &Path, item: &DesktopResourceManifestItem) -> bool {
    target
        .metadata()
        .is_ok_and(|metadata| metadata.len() == item.byte_size)
        && fs::read_to_string(marker_path(target)).is_ok_and(|value| value.trim() == item.sha256)
}
fn write_verified_marker(target: &Path, item: &DesktopResourceManifestItem) -> Result<(), String> {
    fs::write(marker_path(target), format!("{}\n", item.sha256))
        .map_err(|error| format!("写入资源验证标记失败：{error}"))
}
fn file_matches(path: &Path, item: &DesktopResourceManifestItem) -> Result<bool, String> {
    Ok(path
        .metadata()
        .map(|metadata| metadata.len() == item.byte_size)
        .unwrap_or(false)
...
fn session_average_speed(
    ...
    if elapsed_millis < 750 || session_bytes < 64 * 1024 {
        return 0;
    }
...
fn install_destination(item: &DesktopResourceManifestItem, settings: &DesktopSettings) -> PathBuf {
    match item.kind.as_str() {
        ...
        "model" => Path::new(&settings.model_root)
            .join(item.install_directory.as_deref().unwrap_or("checkpoints"))
            .join(&item.file_name),
        "lora" => Path::new(&settings.model_root)
            .join("loras")
            .join(&item.file_name),
...
fn collect_model_registrations(
    settings: &DesktopSettings,
    items: &[DesktopResourceManifestItem],
) -> Result<Vec<LocalModelRegistration>, String> {
    let mut groups: HashMap<&str, Vec<&DesktopResourceManifestItem>> = HashMap::new();
    for item in items
        .iter()
        .filter(|item| item.model_registration.is_some())
    {
        groups
            .entry(item.model_registration.as_ref().unwrap().group_id.as_str())
            .or_default()
            .push(item);
    }
    ...
    for group in groups.into_values() {
        if !group
            .iter()
            .all(|item| installed_resource_matches(item, settings))
        {
            continue;
        }
        let primary = group
            .iter()
            .find(|item| {
                item.model_registration
                    .as_ref()
                    .is_some_and(|registration| registration.role == "primary")
            })
            .ok_or_else(|| "模型组合缺少主文件".to_string())?;
        let metadata = install_destination(primary, settings)
            .metadata()
            .map_err(|error| format!("读取已安装底模失败：{error}"))?;
        let primary_registration = primary.model_registration.as_ref().unwrap();
        let text_encoder = group.iter().find(|item| {
            item.model_registration
                .as_ref()
                .is_some_and(|registration| registration.role == "text_encoder")
        });
        let vae = group.iter().find(|item| {
            item.model_registration
                .as_ref()
                .is_some_and(|registration| registration.role == "vae")
        });
        registrations.push(LocalModelRegistration {
            ...
            model_modified_ms: metadata
                .modified()
                .map_err(|error| format!("读取底模修改时间失败：{error}"))?
                .duration_since(UNIX_EPOCH)
                .map_err(|_| "底模修改时间早于系统纪元".to_string())?
                .as_millis() as u64,
            text_encoder_file_name: text_encoder.map(|item| item.file_name.clone()),
            text_encoder_relative_path: text_encoder
                .map(|item| resource_relative_path(item))
                .transpose()?,
...
fn resource_relative_path(item: &DesktopResourceManifestItem) -> Result<String, String> {
    let directory = item
        .install_directory
        .as_deref()
        .ok_or_else(|| "模型资源缺少安装目录".to_string())?;
    Ok(format!("{directory}/{}", item.file_name))
}

fn installed_resource_matches(
    item: &DesktopResourceManifestItem,
    settings: &DesktopSettings,
) -> bool {
    let destination = install_destination(item, settings);
    ...
    if item.archive == "raw"
        && destination
            .metadata()
            .map(|metadata| !metadata.is_file() || metadata.len() != item.byte_size)
            .unwrap_or(true)
    {
        return false;
    }
    ...
    let sha256_matches =
        value.get("sha256").and_then(serde_json::Value::as_str) == Some(item.sha256.as_str());
    let resource_matches =
        value.get("resourceId").and_then(serde_json::Value::as_str) == Some(item.id.as_str());
...
fn install_marker_path(destination: &Path, directory: bool) -> PathBuf {
    if directory {
        destination.join(".drawhime-resource.json")
    } else {
...
fn extract_zip_safely(
    ...
    let mut archive =
        ZipArchive::new(file).map_err(|error| format!("读取资源 ZIP 失败：{error}"))?;
    if archive.len() > 200_000 {
        return Err("资源 ZIP 文件数量超过限制".into());
    }
    ...
    for index in 0..archive.len() {
        ...
        {
            return Err("资源 ZIP 包含链接文件".into());
        }
        let relative = entry
            .enclosed_name()
            .ok_or_else(|| "资源 ZIP 包含路径穿越项".to_string())?;
        validate_windows_relative_path(&relative)?;
        extracted_bytes = extracted_bytes
            .checked_add(entry.size())
            .ok_or_else(|| "资源 ZIP 解压大小溢出".to_string())?;
        if extracted_bytes > maximum_installed_bytes {
            return Err("资源 ZIP 解压大小超过签名清单声明".into());
        }
        let output = staging.join(relative);
        if entry.is_dir() {
            fs::create_dir_all(&output).map_err(|error| format!("创建 ZIP 目录失败：{error}"))?;
            continue;
        }
        ...
        let mut target = OpenOptions::new()
            ...
            .write(true)
            .open(&output)
            .map_err(|error| format!("创建 ZIP 解压文件失败：{error}"))?;
        std::io::copy(&mut entry, &mut target)
            .map_err(|error| format!("解压资源文件失败：{error}"))?;
        target
            .flush()
            .map_err(|error| format!("保存解压资源失败：{error}"))?;
    }
    Ok(())
}

fn extract_7z_safely(
    archive_path: &Path,
    staging: &Path,
    maximum_installed_bytes: u64,
) -> Result<(), String> {
    ...
    sevenz_rust::decompress_file_with_extract_fn(archive_path, staging, |entry, reader, _| {
        ...
        if entry_count > 200_000 {
            return Err(sevenz_rust::Error::other("资源 7z 文件数量超过限制"));
        }
        let relative = validate_7z_entry(entry).map_err(sevenz_rust::Error::other)?;
        extracted_bytes = extracted_bytes
            .checked_add(entry.size())
            .ok_or_else(|| sevenz_rust::Error::other("资源 7z 解压大小溢出"))?;
        if extracted_bytes > maximum_installed_bytes {
        ...
        if written != entry.size() {
            return Err(sevenz_rust::Error::other("资源 7z 解压文件长度不正确"));
        }
        target.flush()?;
        Ok(true)
    })
...
fn validate_7z_archive(path: &Path, maximum_installed_bytes: u64) -> Result<(), String> {
    let archive =
        sevenz_rust::Archive::open(path).map_err(|error| format!("读取资源 7z 失败：{error}"))?;
    if archive.files.len() > 200_000 {
        return Err("资源 7z 文件数量超过限制".into());
    }
    let mut total = 0_u64;
    for entry in &archive.files {
        ...
        total = total
            .checked_add(entry.size())
            .ok_or_else(|| "资源 7z 解压大小溢出".to_string())?;
        if total > maximum_installed_bytes {
            return Err("资源 7z 解压大小超过签名清单声明".into());
        }
    }
    Ok(())
}
...
fn validate_7z_entry(entry: &sevenz_rust::SevenZArchiveEntry) -> Result<PathBuf, String> {
    if entry.is_anti_item() {
        return Err("资源 7z 包含删除条目".into());
    }
    let attributes = entry.windows_attributes();
    let unix_mode = attributes >> 16;
    if attributes & 0x400 != 0 || unix_mode & 0o170000 == 0o120000 {
        return Err("资源 7z 包含链接条目".into());
    }
...
fn validate_downloaded_archive(
    ...
    match item.archive.as_str() {
        ...
        "zip" => {
            ...
            ZipArchive::new(file)
                .map(|_| ())
                .map_err(|error| format!("读取资源 ZIP 失败：{error}"))
        }
...
fn archive_cache_should_recover(item: &DesktopResourceManifestItem, error: &str) -> bool {
    item.archive == "7z"
        && (error.starts_with("解压资源 7z 失败")
            || error.starts_with("资源 7z 文件列表读取失败")
            || error.starts_with("资源 7z 类型列表读取失败")
            || error.starts_with("读取 7z 文件列表失败")
            || error.starts_with("读取 7z 类型列表失败"))
}
    ...
    for auxiliary in [marker_path(cache), partial_path(cache)] {
        if auxiliary.exists() {
            fs::remove_file(&auxiliary)
                .map_err(|error| format!("清理损坏资源缓存状态失败：{error}"))?;
        }
    }
    Ok(())
}
...
fn validate_windows_relative_path(path: &Path) -> Result<(), String> {
    ...
    for component in path.components() {
        let Component::Normal(value) = component else {
            return Err("资源 ZIP 包含非普通相对路径".into());
        };
        ...
        if text.is_empty()
            ...
            || text.contains('\0')
        {
            return Err("资源 ZIP 包含 Windows 不安全文件名".into());
        }
        let stem = text
            .split('.')
            .next()
        ...
        if RESERVED.contains(&stem.as_str()) {
            return Err("资源 ZIP 包含 Windows 保留文件名".into());
        }
    }
    Ok(())
}

fn validate_extracted_resource(
    item: &DesktopResourceManifestItem,
    staging: &Path,
) -> Result<(), String> {
    if item.kind == "runtime" {
    ...
    if item.kind == "captioner" {
        for required in [
            ...
            staging.join("model.onnx"),
            staging.join("selected_tags.csv"),
            staging
                .join("site-packages")
                .join("onnxruntime")
                .join("__init__.py"),
    ...
    if item.kind == "trainer" {
        for required in [
            staging.join("runner.py"),
            staging.join("sd-scripts").join("anima_train_network.py"),
            staging
                .join("sd-scripts")
                .join("networks")
                .join("lora_anima.py"),
...
fn sha256_file(path: &Path) -> Result<String, String> {
    ...
    // 资源校验可能与生成并发，MiB 级缓冲必须位于堆上，避免桌面工作线程栈溢出。
    let mut buffer = vec![0_u8; 1024 * 1024];
    loop {
        let read = reader
            .read(&mut buffer)
            .map_err(|error| format!("计算资源哈希失败：{error}"))?;
        if read == 0 {
            break;
        }
...
fn quarantine_invalid_target(target: &Path) -> Result<(), String> {
    ...
    if marker.exists() {
        fs::remove_file(marker).map_err(|error| format!("清理失效验证标记失败：{error}"))?;
    }
    Ok(())
}
...
fn emit_progress(app: &tauri::AppHandle, view: DesktopResourceDownloadView) {
    let _ = app.emit("desktop-resource-progress", view);
}
fn install_view(
    resource_id: &str,
    status: &str,
...
fn emit_install_progress(app: &tauri::AppHandle, view: DesktopResourceInstallView) {
    let _ = app.emit("desktop-resource-install-progress", view);
}
...
mod tests {
    ...
    fn resource_rejects_multiple_mirror_urls() {
        let mut item = item();
        item.sources.push(DesktopResourceSource {
            kind: "mirror".into(),
            url: "https://mirror-2.example/runtime.zip".into(),
        });
    ...
    fn cached_manifest_preserves_verified_envelope() {
        ...
        let envelope =
            signed_application_envelope(application_item(b"cached-installer"), &signing_key);
    ...
    fn offline_application_rejects_tampered_envelope() {
        ...
        let mut envelope =
            signed_application_envelope(application_item(b"signed-installer"), &signing_key);
        envelope.payload.push(' ');
        let envelope_path = temporary.path().join("update.envelope.json");
        fs::write(
            &envelope_path,
            serde_json::to_vec(&envelope).expect("序列化篡改信封"),
        )
        .expect("写入篡改信封");
        let public_key = BASE64.encode(signing_key.verifying_key().to_bytes());
        assert!(
            verify_application_package(&installer, &envelope_path, "test", &public_key).is_err()
        );
    }
    ...
    fn offline_application_rejects_installer_sha256_mismatch() {
        ...
        fs::write(
            &envelope_path,
            serde_json::to_vec(&envelope).expect("序列化签名信封"),
        )
        .expect("写入签名信封");
        let public_key = BASE64.encode(signing_key.verifying_key().to_bytes());
        assert!(
    ...
    fn range_download_resumes_existing_partial_file() {
        ...
        let source = DesktopResourceSource {
            kind: "mirror".into(),
    ...
    fn range_download_rejects_misaligned_content_range() {
        ...
        .expect_err("错位 Content-Range 必须被拒绝");
    ...
    fn runtime_zip_extracts_into_staging_and_rejects_windows_unsafe_paths() {
        ...
        archive
            .start_file("python_embeded/python.exe", options)
            .expect("创建便携 Python 项");
        archive.write_all(b"python").expect("写入便携 Python");
        archive
            .start_file("ComfyUI/main.py", options)
            .expect("创建 ComfyUI 入口项");
        archive
            .write_all(b"print('comfyui')")
            .expect("写入 ComfyUI 入口");
        archive
            .start_file("bin/worker.txt", options)
            .expect("创建 Runtime 文件项");
        archive
            .write_all(b"runtime-worker")
            .expect("写入 Runtime 文件");
        ...
        assert_eq!(
            fs::read(staging.join("bin/worker.txt")).expect("读取 Runtime 文件"),
            b"runtime-worker"
        );
    ...
    fn lzma_7z_uses_embedded_decoder_without_system_tar() {
        ...
        assert_eq!(
            fs::read_to_string(staging.join("file.txt")).expect("读取解压结果"),
            "this is a file\n"
        );
    }
    ...
    fn atomic_switch_keeps_previous_version_for_rollback() {
        ...
        let backup = switch_atomically(&staging, &destination)
            .expect("原子切换")
            .expect("保留旧版本路径");
        assert_eq!(
            fs::read(destination.join("version.txt")).expect("读取新版本"),
            b"new"
        );
        assert_eq!(
            fs::read(backup.join("version.txt")).expect("读取回滚版本"),
            b"old"
        );
    }
    ...
    fn verified_runtime_cache_installs_and_updates_environment_state() {
        ...
        archive
            .start_file("ComfyUI/main.py", options)
            .expect("创建 ComfyUI 入口");
        archive
            .write_all(b"print('comfyui')")
            .expect("写入 ComfyUI 入口");
        archive
            .start_file("bin/runtime.txt", options)
            .expect("创建 Runtime 程序文件");
        archive
            .write_all(b"verified-runtime")
            .expect("写入 Runtime 程序文件");
        ...
        let result =
            install_cached_resource(&settings, &item, &cache, &|_| {}).expect("安装已验证 Runtime");
    ...
    fn mirror_runtime_7z_is_compatible_with_safe_extractor() {
        let Ok(archive_path) = std::env::var("DRAWHIME_RUNTIME_TEST_ARCHIVE") else {
            return;
        };
        ...
        let result = install_cached_resource(&settings, &item, Path::new(&archive_path), &|_| {})
            .expect("安全安装官方 Runtime");
        assert_eq!(result.status, "installed");
        assert!(installed_resource_matches(&item, &settings));
        assert_eq!(
            crate::environment::inspect_environment(&settings)
                .runtime
                .status,
    ...
    fn published_captioner_zip_installs_all_runtime_files() {
        let Ok(archive_path) = std::env::var("DRAWHIME_CAPTIONER_TEST_ARCHIVE") else {
            return;
        };
        ...
        fs::write(
            runtime_root.join("current/runtime-manifest.json"),
            b"{\"status\":\"ready\"}",
        )
        .expect("写入测试 Runtime 清单");
        let settings = DesktopSettings {
            theme_mode: "system".into(),
            font_scale: 1.1,
            default_privacy: "private".into(),
    ...
    fn published_trainer_zip_installs_anima_training_entry() {
        let Ok(archive_path) = std::env::var("DRAWHIME_TRAINER_TEST_ARCHIVE") else {
            return;
        };
        ...
        .expect("写入测试 Runtime 清单");
        fs::write(
            runtime_root.join("current/python_embeded/python.exe"),
            b"test",
        )
        .expect("写入测试 Python 标记");
        let model_root = temporary.path().join("models");
        for directory in ["diffusion_models", "text_encoders", "vae"] {
            fs::create_dir_all(model_root.join(directory)).expect("创建测试模型目录");
        }
        fs::write(
            model_root.join("diffusion_models/anima.safetensors"),
            b"test",
        )
        .expect("写入测试 DiT");
        fs::write(
            model_root.join("text_encoders/qwen_3_06b_base.safetensors"),
            b"test",
        )
        .expect("写入测试文本编码器");
        fs::write(model_root.join("vae/qwen_image_vae.safetensors"), b"test")
            .expect("写入测试 VAE");
        let settings = DesktopSettings {
            theme_mode: "system".into(),
            font_scale: 1.1,
    ...
    fn installed_anima_resource_group_becomes_one_registered_model() {
        ...
        let definitions = [
            ...
            ("vae", "vae", "vae.safetensors", b"vae".as_slice()),
        ];
        ...
        assert_eq!(
            registrations[0].text_encoder_file_name.as_deref(),
            Some("clip.safetensors")
        );
        assert_eq!(
            registrations[0].vae_file_name.as_deref(),
            Some("vae.safetensors")
        );
    }
    ...
    fn identical_raw_model_resource_is_shared_across_groups() {
        ...
        first.model_registration = Some(crate::models::DesktopResourceModelRegistration {
            group_id: "model.first".into(),
            display_name: "模型一".into(),
            family: "anima".into(),
            workflow_kind: "anima".into(),
            role: "text_encoder".into(),
        });
        install_cached_resource(&settings, &first, &cache, &|_| {}).expect("安装第一组共享组件");
        let mut second = first.clone();
        second.id = "model.second.text-encoder".into();
        second.model_registration = Some(crate::models::DesktopResourceModelRegistration {
            group_id: "model.second".into(),
            display_name: "模型二".into(),
            family: "anima".into(),
            workflow_kind: "anima".into(),
            role: "text_encoder".into(),
        });
