...
impl TrainingScheduler {
    ...
    pub fn start(database_path: PathBuf, app_data_dir: PathBuf, runtime: Arc<RuntimeController>, gpu_workload: Arc<GpuWorkloadCoordinator>, app: AppHandle) -> Result<Self, String> {
        let stopping = Arc::new(AtomicBool::new(false));
        let wake_signal = Arc::new((Mutex::new(false), Condvar::new()));
        let worker_stopping = stopping.clone();
        let worker_signal = wake_signal.clone();
        let worker = thread::Builder::new().name("drawhime-training-scheduler".into()).spawn(move || training_loop(&database_path, &app_data_dir, &runtime, &gpu_workload, &app, &worker_stopping, &worker_signal)).map_err(|error| format!("启动本地训练线程失败：{error}"))?;
...
fn training_loop(database_path: &Path, app_data_dir: &Path, runtime: &RuntimeController, gpu_workload: &Arc<GpuWorkloadCoordinator>, app: &AppHandle, stopping: &AtomicBool, wake_signal: &(Mutex<bool>, Condvar)) {
...
fn execute_job(database: &Connection, app_data_dir: &Path, runtime: &RuntimeController, app: &AppHandle, stopping: &AtomicBool, job: TrainingExecution) {
    ...
    let terminal = match result {
        ...
        Err(TrainingStop::Failed { message, oom }) => training::finish_failed(database, &job, &message, oom.then(|| oom_suggestion(&job))),
    };
    if terminal.is_ok() { training::emit_job(database, app, &job.id); }
}

fn execute_runner(database: &Connection, app_data_dir: &Path, runtime: &RuntimeController, app: &AppHandle, stopping: &AtomicBool, job: &TrainingExecution) -> Result<crate::storage::LocalLoraRegistration, TrainingStop> {
    ...
    let stdout = child.stdout.take().ok_or_else(|| failed("Trainer 没有标准输出"))?;
    ...
    loop {
        if stopping.load(Ordering::SeqCst) {
            ...
            let _ = stdout_reader.join(); let _ = stderr_reader.join(); let _ = fs::remove_file(&request_path);
        ...
        match receiver.recv_timeout(Duration::from_millis(500)) {
            Ok(Ok(line)) => {
                let event: TrainerEvent = serde_json::from_str(&line).map_err(|_| failed("Trainer 返回了无效进度"))?;
                match event.kind.as_str() {
                    "progress" => { training::update_progress(database, &job.id, event.progress.unwrap_or(1), event.current_epoch.unwrap_or(0)).map_err(failed)?; training::emit_job(database, app, &job.id); }
                    "result" => output_path = event.path.map(PathBuf::from),
                    "error" => runner_error = Some((event.message.unwrap_or_else(|| "本地训练失败".into()), event.oom.unwrap_or(false))),
                    _ => return Err(failed("Trainer 返回了未知事件")),
                }
            }
            Ok(Err(error)) => { kill_process_tree(&mut child); return Err(failed(error)); }
            Err(mpsc::RecvTimeoutError::Timeout) => if child.try_wait().ok().flatten().is_some() { break; },
            Err(mpsc::RecvTimeoutError::Disconnected) => break,
        }
    }
    let status = child.wait().map_err(|error| failed(format!("等待 Trainer 退出失败：{error}")))?;
    let _ = stdout_reader.join();
    let stderr = stderr_reader.join().unwrap_or_default();
    let _ = fs::remove_file(&request_path);
    if !status.success() {
        let (message, oom) = runner_error.unwrap_or_else(|| classify_runner_error(&stderr, &component));
...
fn validate_execution_files(app_data_dir: &Path, model_root: &Path, job: &TrainingExecution) -> Result<(), String> {
    validate_snapshot(&model_root.join(&job.model_relative_path), &job.model_sha256, Some(job.model_byte_size), Some(job.model_modified_ms))?;
    validate_snapshot(&model_root.join(&job.text_encoder_relative_path), &job.text_encoder_sha256, None, None)?;
    validate_snapshot(&model_root.join(&job.vae_relative_path), &job.vae_sha256, None, None)?;
    for asset in &job.assets { validate_snapshot(&app_data_dir.join(&asset.relative_path), &asset.sha256, Some(asset.byte_size), None)?; }
    Ok(())
}

fn validate_snapshot(path: &Path, expected_hash: &str, expected_size: Option<u64>, expected_modified_ms: Option<u64>) -> Result<(), String> {
    let metadata = path.metadata().map_err(|_| "训练任务快照文件缺失".to_string())?;
    if !metadata.is_file() || expected_size.is_some_and(|value| value != metadata.len()) { return Err("训练任务快照文件大小已经变化".into()); }
    if expected_modified_ms.is_some_and(|value| modified_millis(&metadata).ok() != Some(value)) { return Err("训练底模修改时间已经变化".into()); }
    if sha256_file(path)? != expected_hash { return Err("训练任务快照文件 SHA-256 已变化".into()); }
    Ok(())
}
...
fn validate_output_path(workspace: &Path, output: &Path) -> Result<(), String> {
    let root = fs::canonicalize(workspace).map_err(|error| format!("读取训练工作目录失败：{error}"))?;
    let output = fs::canonicalize(output).map_err(|_| "训练 LoRA 产物不存在".to_string())?;
    if !output.starts_with(&root) || output.extension().is_none_or(|value| !value.to_string_lossy().eq_ignore_ascii_case("safetensors")) { return Err("Trainer 返回了工作目录外的产物".into()); }
    Ok(())
}
...
fn oom_suggestion(job: &TrainingExecution) -> DesktopTrainingSuggestionView {
    let resolution = match job.parameters.resolution { value if value > 1024 => Some(1024), value if value > 768 => Some(768), value if value > 512 => Some(512), _ => None };
    let rank = match job.parameters.rank { value if value > 32 => Some(32), value if value > 16 => Some(16), value if value > 8 => Some(8), _ => None };
    DesktopTrainingSuggestionView { message: "GPU 显存不足；优先应用建议分辨率，其次降低 Rank，并关闭其他占用 GPU 的程序。".into(), resolution, rank }
}

fn write_request(path: &Path, request: &TrainerRequest) -> Result<(), String> {
    let temporary = path.with_extension("json.writing");
    let mut file = File::create(&temporary).map_err(|error| format!("创建 Trainer 请求失败：{error}"))?;
    serde_json::to_writer(&mut file, request).map_err(|error| format!("序列化 Trainer 请求失败：{error}"))?;
    file.flush().and_then(|_| file.sync_all()).map_err(|error| format!("保存 Trainer 请求失败：{error}"))?;
...
fn read_limited_stderr(stderr: Option<impl Read>) -> String {
    ...
    let mut bytes = Vec::new(); let _ = stderr.by_ref().take(MAX_RUNNER_ERROR_BYTES as u64).read_to_end(&mut bytes); String::from_utf8_lossy(&bytes).into_owned()
}
...
fn classify_runner_error(stderr: &str, component: &TrainerComponent) -> (String, bool) {
    ...
    let message = if oom { "GPU 显存不足，训练进程已安全停止".into() } else { text.lines().last().unwrap_or("Trainer 进程异常退出").trim().chars().take(800).collect() };
    (message, oom)
}
