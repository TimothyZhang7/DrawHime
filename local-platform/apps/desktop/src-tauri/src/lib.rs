...
use models::{DesktopAiAnalyzeInput, DesktopAiAnalyzeView, DesktopAiSettings, DesktopAiSettingsUpdate, DesktopBootstrapView, DesktopCaptionJobCreateInput, DesktopCaptionJobView, DesktopEnvironmentReport, DesktopLocalJobCreateInput, DesktopLocalJobView, DesktopLocalLoraImportInput, DesktopLocalLoraView, DesktopLocalModelImportInput, DesktopLocalModelView, DesktopOfflineUpdateImportInput, DesktopResourceCatalogView, DesktopResourceDownloadView, DesktopResourceInstallView, DesktopRuntimeStatusView, DesktopSettings, DesktopSoftwareUpdateView, DesktopTrainingAssetDeleteInput, DesktopTrainingCaptionUpdateInput, DesktopTrainingDatasetCreateInput, DesktopTrainingDatasetIdInput, DesktopTrainingDatasetView, DesktopTrainingImagesAddInput, DesktopTrainingJobCreateInput, DesktopTrainingJobView, DesktopTrainingTagTranslationInput, DesktopTrainingTagTranslationView, DesktopTrainingTriggerWordsUpdateInput, DesktopWebsiteLoraView, DesktopWebsiteModelView, GalleryPublicationInput, GallerySyncItem};
...
async fn desktop_apply_software_update(app: tauri::AppHandle, state: State<'_, DesktopState>) -> Result<DesktopSoftwareUpdateView, String> {
    let database_path = state.database_path.clone();
    let app_data_dir = state.app_data_dir.clone();
    let relaunch_path = env::current_exe().map_err(|error| format!("读取当前程序路径失败：{error}"))?;
    let view = tauri::async_runtime::spawn_blocking(move || software_update::apply(&database_path, &app_data_dir, &relaunch_path)).await.map_err(|error| format!("软件更新应用任务异常：{error}"))??;
...
async fn desktop_rollback_software_update(app: tauri::AppHandle, state: State<'_, DesktopState>) -> Result<DesktopSoftwareUpdateView, String> {
    ...
    let app_data_dir = state.app_data_dir.clone();
    let relaunch_path = env::current_exe().map_err(|error| format!("读取当前程序路径失败：{error}"))?;
    let view = tauri::async_runtime::spawn_blocking(move || software_update::rollback(&database_path, &app_data_dir, &relaunch_path)).await.map_err(|error| format!("软件回滚任务异常：{error}"))??;
    let exit_app = app.clone();
    std::thread::spawn(move || {
        std::thread::sleep(std::time::Duration::from_millis(600));
        exit_app.exit(0);
...
#[tauri::command]
async fn desktop_delete_training_asset(state: State<'_, DesktopState>, input: DesktopTrainingAssetDeleteInput) -> Result<DesktopTrainingDatasetView, String> {
    let database_path = state.database_path.clone();
    let app_data_dir = state.app_data_dir.clone();
    tauri::async_runtime::spawn_blocking(move || {
    ...
    }).await.map_err(|error| format!("删除训练图片任务异常：{error}"))?
}

/** 设备会话在 Rust 核心内调用真实标签翻译接口，密钥不会返回页面。 */
#[tauri::command]
async fn desktop_translate_training_tags(input: DesktopTrainingTagTranslationInput) -> Result<DesktopTrainingTagTranslationView, String> {
    tauri::async_runtime::spawn_blocking(move || website_tag_translation::translate(input)).await.map_err(|error| format!("标签翻译任务异常：{error}"))?
}
