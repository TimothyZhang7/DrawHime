...
pub fn store<T: Serialize>(app_data_dir: &Path, namespace: &str, value: &T) -> Result<(), String> {
    ...
    let bytes =
        serde_json::to_vec(value).map_err(|error| format!("序列化网站仓库缓存失败：{error}"))?;
    if bytes.is_empty() || bytes.len() as u64 > MAXIMUM_CATALOG_BYTES {
        return Err("网站仓库缓存大小不正确".into());
    }
    let temporary = path.with_extension("json.part");
    let previous = path.with_extension("json.previous");
    fs::write(&temporary, bytes).map_err(|error| format!("写入网站仓库缓存失败：{error}"))?;
    if previous.exists() {
        fs::remove_file(&previous).map_err(|error| format!("清理网站仓库旧缓存失败：{error}"))?;
    }
    if path.exists() {
        fs::rename(&path, &previous).map_err(|error| format!("暂存网站仓库旧缓存失败：{error}"))?;
    }
    ...
    if previous.exists() {
        fs::remove_file(previous).map_err(|error| format!("清理网站仓库备份失败：{error}"))?;
    }
    Ok(())
}
...
pub fn load<T: DeserializeOwned>(app_data_dir: &Path, namespace: &str) -> Result<T, String> {
    ...
    let metadata = path
        .metadata()
        .map_err(|_| "本机尚无可用的网站仓库缓存".to_string())?;
    if !metadata.is_file() || metadata.len() == 0 || metadata.len() > MAXIMUM_CATALOG_BYTES {
        return Err("网站仓库缓存文件大小不正确".into());
    }
...
mod tests {
    ...
    fn catalog_cache_round_trip_is_atomic() {
        ...
        assert!(!directory
            .path()
            .join("catalog-cache/models.json.previous")
            .exists());
    }
}
