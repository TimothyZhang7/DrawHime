...
pub fn cache_images(client: &Client, token: &str, app_data_dir: &Path, namespace: &str, images: &[WebsiteImageRef], force_refresh: bool) -> Vec<String> {
    ...
    for (batch_index, batch) in images.chunks(4).enumerate() {
        let resolved = std::thread::scope(|scope| {
            batch.iter().enumerate().map(|(image_index, image)| {
                // 手动刷新只强制更新首图；新示例因本机尚无缓存仍会下载，既保证封面实时又避免重复传输全部图库。
                let refresh_image = force_refresh && batch_index == 0 && image_index == 0;
                scope.spawn(move || cache_cover_inner(client, token, app_data_dir, namespace, image, refresh_image))
            }).collect::<Vec<_>>().into_iter().map(|handle| handle.join().ok().and_then(Result::ok)).collect::<Vec<_>>()
        });
...
fn cache_cover_inner(client: &Client, token: &str, app_data_dir: &Path, namespace: &str, image: &WebsiteImageRef, force_refresh: bool) -> Result<PathBuf, String> {
    ...
    let cached = ["webp", "png", "jpg"].into_iter().map(|extension| directory.join(format!("{safe_id}.{extension}"))).find(|path| path.metadata().is_ok_and(|metadata| metadata.len() > 0));
    if !force_refresh { if let Some(path) = cached.clone() { return Ok(path); } }
    ...
    match refreshed {
        Ok(target) => { if let Some(previous) = cached.filter(|path| path != &target) { let _ = fs::remove_file(previous); } Ok(target) }
...
fn replace_cached_file(temporary: &Path, target: &Path) -> Result<(), String> {
    ...
    if target.exists() { fs::rename(target, &backup).map_err(|error| format!("暂存旧仓库封面失败：{error}"))?; }
