...
impl DesktopState {
    ...
    pub fn initialize(app_data_dir: &Path, picture_dir: &Path) -> Result<Self, String> {
        ...
        connection.execute("INSERT OR IGNORE INTO desktop_settings (id, default_privacy, model_root, output_root, runtime_root, upload_concurrency, wifi_only, bandwidth_limit_kib, updated_at) VALUES (1, 'private', ?1, ?2, ?3, 2, 0, NULL, ?4)", params![path_text(&model_root), path_text(&output_root), path_text(&runtime_root), Utc::now().to_rfc3339()]).map_err(|error| format!("写入默认设置失败：{error}"))?;
    ...
    pub fn load_settings(&self) -> Result<DesktopSettings, String> {
        ...
        database.query_row("SELECT default_privacy, model_root, output_root, runtime_root, upload_concurrency, wifi_only, bandwidth_limit_kib FROM desktop_settings WHERE id=1", [], |row| Ok(DesktopSettings { default_privacy: row.get(0)?, model_root: row.get(1)?, output_root: row.get(2)?, runtime_root: row.get(3)?, upload_concurrency: row.get(4)?, wifi_only: row.get::<_, i64>(5)? != 0, bandwidth_limit_kib: row.get(6)? })).map_err(|error| format!("读取桌面设置失败：{error}"))
    }
    ...
    pub fn save_settings(&self, settings: DesktopSettings) -> Result<DesktopSettings, String> {
        ...
        let database = self.database.lock().map_err(|_| "桌面数据库锁已损坏".to_string())?;
        database.execute("UPDATE desktop_settings SET default_privacy=?1, model_root=?2, output_root=?3, runtime_root=?4, upload_concurrency=?5, wifi_only=?6, bandwidth_limit_kib=?7, updated_at=?8 WHERE id=1", params![settings.default_privacy, settings.model_root, settings.output_root, settings.runtime_root, settings.upload_concurrency, settings.wifi_only, settings.bandwidth_limit_kib, Utc::now().to_rfc3339()]).map_err(|error| format!("保存桌面设置失败：{error}"))?;
        drop(database);
        self.load_settings()
    }
