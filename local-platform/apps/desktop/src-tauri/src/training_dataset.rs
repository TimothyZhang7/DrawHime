...
use crate::models::{
    DesktopTrainingAssetDeleteInput, DesktopTrainingAssetView, DesktopTrainingCaptionUpdateInput,
...
pub fn create_dataset(
    ...
    if !matches!(input.r#type.as_str(), "character" | "style" | "concept") {
...
pub fn delete_asset(
    ...
    let referenced: bool = database
        .query_row(
            "SELECT EXISTS(SELECT 1 FROM local_training_job_assets WHERE asset_id=?1)",
            [&input.asset_id],
            |row| row.get(0),
        )
        .map_err(|error| format!("检查训练图片快照失败：{error}"))?;
    if referenced {
        return Err("该图片已用于训练任务快照，为保留任务审计不可删除".into());
    }
    ...
    if captioning {
        return Err("当前训练集仍有打标任务运行，请完成或取消后再删除图片".into());
    }
    let dataset_root = app_data_dir.join("datasets").join(&input.dataset_id);
