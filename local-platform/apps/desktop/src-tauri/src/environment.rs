...
use crate::{models::{CapabilityView, CpuView, DesktopEnvironmentReport, DesktopSettings, EnvironmentIssue, GpuView, MemoryView, OsView, RuntimeView, WindowsSystemProbe}, process::hide_window, trainer};
use chrono::Utc;
...
pub fn inspect_environment(settings: &DesktopSettings) -> DesktopEnvironmentReport {
    ...
    if runtime.installed && !trainer_ready { issues.push(issue("trainer_missing", "warning", "本地 LoRA Trainer 需要修复", "训练集和 Caption 仍可整理；请安装当前签名 Trainer 组件，旧协议组件不会继续执行训练。", "前往资源安装")); }
