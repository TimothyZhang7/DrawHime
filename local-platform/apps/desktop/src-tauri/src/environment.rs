...
use crate::{
    models::{
        ...
        GpuView, MemoryView, OsView, RuntimeView, WindowsSystemProbe,
    },
    process::hide_window,
    trainer,
};
...
pub fn inspect_environment(settings: &DesktopSettings) -> DesktopEnvironmentReport {
    ...
    let os_supported = cfg!(target_os = "windows")
        && windows_build_supported(system.os_version.as_deref(), system.os_build);
    ...
    let (gpu_supported, training_gpu_supported, low_free_memory) =
        evaluate_gpu_state(&gpus, nvidia_hardware_detected, &mut issues);
    ...
    if runtime.installed && !trainer_ready {
        issues.push(issue(
            "trainer_missing",
            "warning",
    ...
    let status = if !os_supported || !gpu_supported || runtime.status == "broken" {
        "blocked"
...
fn evaluate_gpu_state(
    gpus: &[GpuView],
    nvidia_hardware_detected: bool,
    issues: &mut Vec<EnvironmentIssue>,
) -> (bool, bool, bool) {
    let driver_supported = gpus
        .iter()
        .any(|gpu| nvidia_driver_supported(&gpu.driver_version));
    let generation_supported = driver_supported
        && gpus
            .iter()
            .any(|gpu| gpu.memory_total_bytes >= MINIMUM_GPU_MEMORY_BYTES);
    let training_supported = driver_supported
        && gpus
            .iter()
            .any(|gpu| gpu.memory_total_bytes >= MINIMUM_TRAINING_GPU_MEMORY_BYTES);
    let low_free_memory = gpus
        .first()
        .map(|gpu| gpu.memory_free_bytes < 1024 * MIB)
        .unwrap_or(true);
    if gpus.is_empty() {
        ...
        issues.push(issue(
            ...
            "critical",
            "GPU 显存低于首版运行要求",
            "当前检测到的 NVIDIA GPU 均少于 6GB 独立显存，生成和训练保持暂停。",
            "查看显存要求",
        ));
        ...
        issues.push(issue(
            ...
            "warning",
            "GPU 显存仅支持本地生成",
            "当前 GPU 可用于本地生成，但少于 8GB 独立显存，LoRA 训练保持暂停。",
            "查看训练显存要求",
        ));
    }
...
fn windows_system_probe() -> WindowsSystemProbe {
    if cfg!(not(target_os = "windows")) {
        return WindowsSystemProbe {
            ...
            nvidia_adapter_names: Vec::new(),
            disks: Vec::new(),
        };
    }
    let script = r#"[Console]::OutputEncoding=[Text.UTF8Encoding]::new();$os=Get-CimInstance Win32_OperatingSystem;$cpu=Get-CimInstance Win32_Processor|Select-Object -First 1;$video=@(Get-CimInstance Win32_VideoController);$nvidiaAdapters=@($video|Where-Object{$_.Name -match 'NVIDIA'}|ForEach-Object{[string]$_.Name});$displayAdapters=@($video|ForEach-Object{$name=[string]$_.Name;$compatibility=[string]$_.AdapterCompatibility;$vendor=if("$name $compatibility" -match 'NVIDIA'){'NVIDIA'}elseif("$name $compatibility" -match 'AMD|Advanced Micro Devices|Radeon'){'AMD'}elseif("$name $compatibility" -match 'Intel'){'Intel'}else{if($compatibility){$compatibility}else{'未知'}};[ordered]@{name=$name;vendor=$vendor;cudaSupported=($vendor -eq 'NVIDIA')}});$disks=@(Get-CimInstance Win32_LogicalDisk -Filter 'DriveType=3'|ForEach-Object{[ordered]@{name=$_.DeviceID;fileSystem=[string]$_.FileSystem;totalBytes=[uint64]$_.Size;availableBytes=[uint64]$_.FreeSpace}});[ordered]@{osName=[string]$os.Caption;osVersion=[string]$os.Version;osBuild=[uint64]$os.BuildNumber;cpuName=[string]$cpu.Name;totalMemoryBytes=[uint64]$os.TotalVisibleMemorySize*1024;availableMemoryBytes=[uint64]$os.FreePhysicalMemory*1024;virtualTotalBytes=[uint64]$os.TotalVirtualMemorySize*1024;displayAdapters=$displayAdapters;nvidiaAdapterNames=$nvidiaAdapters;disks=$disks}|ConvertTo-Json -Compress -Depth 4"#;
    ...
    String::from_utf8_lossy(&output.stdout)
        ...
        .filter_map(|line| {
            ...
            Some(GpuView {
                ...
                compute_capability: non_empty(columns[6]),
...
fn inspect_runtime(root: &str) -> RuntimeView {
    ...
    let status = fs::read_to_string(&manifest_path)
        .ok()
...
fn windows_build_supported(version: Option<&str>, build: Option<u64>) -> bool {
    version.is_some_and(|value| value.starts_with("10."))
        && build.is_some_and(|value| value >= 17763)
}
...
mod tests {
    ...
    fn gpu_gate_explains_missing_low_memory_and_generation_only_devices() {
        let mut issues = Vec::new();
        assert_eq!(
            evaluate_gpu_state(&[], false, &mut issues),
            (false, false, true)
        );
        ...
        issues.clear();
        assert_eq!(
            evaluate_gpu_state(&[], true, &mut issues),
            (false, false, true)
        );
        assert!(issues
            .iter()
        ...
        let gpu = |total_mib: u64, free_mib: u64, driver_version: &str| GpuView {
            ...
            compute_capability: Some("test".into()),
            temperature_celsius: Some(0.0),
            utilization_percent: Some(0.0),
        };
        issues.clear();
        assert_eq!(
            evaluate_gpu_state(&[gpu(8_188, 8_000, "560.75")], true, &mut issues),
            (false, false, false)
        );
        assert!(issues
            .iter()
            .any(|issue| issue.code == "nvidia_driver_too_old"));

        issues.clear();
        assert_eq!(
            evaluate_gpu_state(&[gpu(4_092, 4_000, "560.76")], true, &mut issues),
            (false, false, false)
        );
        assert!(issues
            .iter()
        ...
        issues.clear();
        assert_eq!(
            evaluate_gpu_state(&[gpu(6_140, 6_000, "560.76")], true, &mut issues),
            (true, false, false)
        );
        assert!(issues
            .iter()
            .any(|issue| issue.code == "training_gpu_memory_insufficient"));

        issues.clear();
        assert_eq!(
            evaluate_gpu_state(&[gpu(8_188, 8_000, "596.21")], true, &mut issues),
            (true, true, false)
        );
