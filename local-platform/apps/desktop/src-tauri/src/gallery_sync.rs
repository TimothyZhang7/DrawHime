...
impl GallerySyncScheduler {
    ...
    pub fn start(database_path: PathBuf, app: AppHandle) -> Result<Self, String> {
        ...
        // 同步包含哈希、分片和 TLS 调用，显式栈空间避免高负载时依赖平台默认值。
        let worker = thread::Builder::new()
            .name("drawhime-gallery-sync".into())
        ...
        let result = Connection::open(database_path)
            ...
            .and_then(|database| {
                database
                    ...
                    .map_err(|error| format!("初始化图库同步数据库失败：{error}"))?;
                process_next(&database, app)
            });
        if let Err(error) = result {
            eprintln!("图库同步线程：{error}");
        }
        let (lock, condition) = signal;
        if let Ok(pending) = lock.lock() {
            let _ = condition.wait_timeout_while(pending, Duration::from_secs(3), |value| !*value);
        }
        if let Ok(mut pending) = lock.lock() {
            *pending = false;
        }
...
fn process_next(database: &Connection, app: &AppHandle) -> Result<(), String> {
    ...
    let auto_upload = database
        .query_row(
            "SELECT auto_upload FROM desktop_settings WHERE id=1",
            [],
            |row| Ok(row.get::<_, i64>(0)? != 0),
        )
        .map_err(|error| format!("读取图库自动上传设置失败：{error}"))?;
    if !auto_upload {
        return Ok(());
    }
    ...
    match synchronize(database, &publication) {
        Ok(upload) => {
            let status = if upload.status == "remote_deleted" {
                "remote_deleted"
            } else {
                "synced"
            };
        ...
        Err(SyncFailure::WaitingNetwork(message)) => schedule(
            database,
            &publication,
            "waiting_network",
            message,
        ...
        Err(SyncFailure::Retryable(message)) => {
            let delay = (5_u64
                .saturating_mul(2_u64.saturating_pow(publication.retry_count.min(6))))
            .min(300);
            schedule(
                database,
                &publication,
        ...
        Err(SyncFailure::Final(message)) => {
            schedule(database, &publication, "failed_final", message, true, 0)?
        }
    }
...
fn synchronize(
...
) -> Result<UploadView, SyncFailure> {
    if wifi_only(database) && !wifi_connected() {
        return Err(SyncFailure::WaitingNetwork(
            "当前未连接 Wi-Fi，已按本机设置暂停图库同步".into(),
        ));
    }
    let session = match auth::authenticated_session() {
        Ok(Some(session)) => session,
        Ok(None) => {
            return Err(SyncFailure::WaitingAuth(
                "请先连接该作品所属的绘图姬账号".into(),
            ))
        }
        Err(DesktopSessionError::Network) => {
            return Err(SyncFailure::WaitingNetwork("账号服务当前不可达".into()))
        }
    ...
    if let (Some(issuer), Some(subject)) = (&publication.owner_issuer, &publication.owner_subject) {
        if issuer != &session.identity.issuer || subject != &session.identity.subject {
            return Err(SyncFailure::WaitingAuth(
                "该作品已绑定其他绘图姬账号".into(),
            ));
        }
    ...
    let client = online_client_builder()
        ...
        .build()
        .map_err(|error| SyncFailure::Retryable(format!("创建图库网络客户端失败：{error}")))?;
    let mut upload = if let Some(upload_id) = &publication.server_upload_id {
        request_upload(
            &client,
            "GET",
            &format!("/v1/desktop/gallery/uploads/{upload_id}"),
            &session.token,
            None,
        )?
    } else {
    ...
    if upload.status == "published" || upload.status == "remote_deleted" {
        return Ok(upload);
    }
    let mut file = File::open(&publication.artifact_path)
        .map_err(|_| SyncFailure::Final("本地原图已经不存在".into()))?;
    ...
    while upload.received_bytes < total {
        ...
        let wanted = usize::try_from((total - upload.received_bytes).min(chunk_bytes as u64))
            .map_err(|_| SyncFailure::Final("作品分片长度不正确".into()))?;
        let mut chunk = vec![0_u8; wanted];
        file.seek(SeekFrom::Start(upload.received_bytes))
            .map_err(|error| SyncFailure::Final(format!("定位本地原图失败：{error}")))?;
        file.read_exact(&mut chunk)
            .map_err(|error| SyncFailure::Final(format!("读取本地原图失败：{error}")))?;
    ...
    database
        .execute(
            "UPDATE gallery_sync_queue SET status='committing',updated_at=?2 WHERE id=?1",
            params![publication.id, Utc::now().to_rfc3339()],
        )
        .map_err(|error| SyncFailure::Retryable(format!("保存图库提交状态失败：{error}")))?;
    request_upload(
        &client,
        "POST",
        &format!("/v1/desktop/gallery/uploads/{}/complete", upload.id),
        &session.token,
        None,
    )
}
...
fn create_upload(
    ...
    let loras = read_lora_parameters(database, &publication.local_task_id)
    ...
    let file_name = Path::new(&publication.artifact_path)
        ...
        .unwrap_or("drawhime-desktop.webp");
    let body = json!({
...
fn request_upload(
    ...
    let builder = match method {
        "POST" => client.post(auth::api_url(path)),
        _ => client.get(auth::api_url(path)),
    };
...
fn put_chunk(
    ...
    parse_upload_response(
        client
            .put(auth::api_url(&format!(
                "/v1/desktop/gallery/uploads/{upload_id}"
            )))
            .bearer_auth(token)
...
fn parse_upload_response(
    ...
    let response = result.map_err(|error| {
        if error.is_connect() || error.is_timeout() {
            SyncFailure::WaitingNetwork("图库服务连接超时".into())
        } else {
            SyncFailure::Retryable("图库网络请求失败".into())
        }
    })?;
    let status = response.status();
    let payload: ApiEnvelope<UploadView> = response
        .json()
        .map_err(|_| SyncFailure::Retryable("图库服务返回格式不正确".into()))?;
    if status == StatusCode::UNAUTHORIZED || status == StatusCode::FORBIDDEN {
        return Err(SyncFailure::WaitingAuth(
            payload
                .message
                .unwrap_or_else(|| "桌面账号授权已失效".into()),
        ));
    }
    if !status.is_success() || !payload.ok {
        let message = payload
            .message
            .or(payload.code)
            .unwrap_or_else(|| format!("图库服务 HTTP {}", status.as_u16()));
        return Err(
    ...
    payload
        .data
        .ok_or_else(|| SyncFailure::Retryable("图库服务没有返回上传状态".into()))
}
...
fn verify_artifact(publication: &PendingPublication) -> Result<(), SyncFailure> {
    let metadata = fs::metadata(&publication.artifact_path)
        .map_err(|_| SyncFailure::Final("本地原图已经不存在".into()))?;
    if !metadata.is_file() || metadata.len() != publication.byte_size {
        return Err(SyncFailure::Final("本地原图字节数已经变化".into()));
    }
    let file = File::open(&publication.artifact_path)
        .map_err(|_| SyncFailure::Final("本地原图读取失败".into()))?;
    ...
    if hex::encode(hasher.finalize()) != publication.artifact_sha256 {
        return Err(SyncFailure::Final("本地原图 SHA-256 已经变化".into()));
    }
    Ok(())
}
...
fn throttle(database: &Connection, bytes: u64, started: Instant) {
    let limit: Option<u64> = database
        .query_row(
            "SELECT bandwidth_limit_kib FROM desktop_settings WHERE id=1",
            [],
            |row| row.get(0),
        )
        .ok()
...
fn wifi_only(database: &Connection) -> bool {
    database
        .query_row(
            "SELECT wifi_only FROM desktop_settings WHERE id=1",
            [],
            |row| row.get::<_, i64>(0),
        )
        .map(|value| value != 0)
...
fn wifi_connected() -> bool {
    let output = hide_window(&mut std::process::Command::new("netsh"))
...
fn emit_item(database: &Connection, app: &AppHandle, id: &str) -> Result<(), String> {
    ...
    app.emit("desktop-gallery-sync-updated", item)
        .map_err(|error| format!("发送图库同步事件失败：{error}"))
}
