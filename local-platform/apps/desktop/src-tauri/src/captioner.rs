...
impl CaptionScheduler {
    ...
    pub fn start(database_path: PathBuf, app_data_dir: PathBuf, app: AppHandle) -> Result<Self, String> {
        ...
        let worker = thread::Builder::new()
            ...
            .spawn(move || caption_loop(&database_path, &app_data_dir, &app, &worker_stopping, &worker_signal))
...
pub fn create_job(database: &mut Connection, input: DesktopCaptionJobCreateInput) -> Result<DesktopCaptionJobView, String> {
    validate_create_input(&input)?;
    let transaction = database.transaction().map_err(|error| format!("开启打标任务事务失败：{error}"))?;
    let dataset_exists: bool = transaction.query_row("SELECT EXISTS(SELECT 1 FROM local_training_datasets WHERE id=?1)", [&input.dataset_id], |row| row.get(0)).map_err(|error| format!("读取训练集失败：{error}"))?;
    if !dataset_exists { return Err("训练集不存在".into()); }
    ...
    if active { return Err("当前训练集已有打标任务正在排队或运行".into()); }
    let targets = select_targets(&transaction, &input)?;
    if targets.is_empty() { return Err("当前训练集没有需要自动打标的图片；人工 Caption 会保持不变".into()); }
    ...
    transaction.execute("UPDATE local_training_assets SET confirmed=0 WHERE dataset_id=?1", [&input.dataset_id]).and_then(|_| transaction.execute("UPDATE local_training_datasets SET status='draft',updated_at=?2 WHERE id=?1", params![input.dataset_id,now])).map_err(|error| format!("重置训练集确认状态失败：{error}"))?;
    transaction.commit().map_err(|error| format!("提交打标任务失败：{error}"))?;
...
pub fn list_jobs(database: &Connection) -> Result<Vec<DesktopCaptionJobView>, String> {
    let mut statement = database.prepare("SELECT id FROM local_caption_jobs ORDER BY created_at DESC LIMIT 100").map_err(|error| format!("读取打标任务列表失败：{error}"))?;
    let ids = statement.query_map([], |row| row.get::<_, String>(0)).map_err(|error| format!("查询打标任务列表失败：{error}"))?.collect::<Result<Vec<_>, _>>().map_err(|error| format!("解析打标任务列表失败：{error}"))?;
    ids.into_iter().map(|id| read_job(database, &id)?.ok_or_else(|| "打标任务读取期间消失".into())).collect()
}
...
pub fn cancel_job(database: &Connection, id: &str) -> Result<DesktopCaptionJobView, String> {
    ...
    let now = Utc::now().to_rfc3339();
    let transaction = database.unchecked_transaction().map_err(|error| format!("开启取消打标任务事务失败：{error}"))?;
    let status: Option<String> = transaction.query_row("SELECT status FROM local_caption_jobs WHERE id=?1", [id], |row| row.get(0)).optional().map_err(|error| format!("读取打标任务失败：{error}"))?;
    let Some(status) = status else { return Err("打标任务不存在".into()); };
    if status == "queued" {
        ...
        transaction.execute("UPDATE local_caption_jobs SET cancel_requested=1,updated_at=?2 WHERE id=?1", params![id,now]).map_err(|error| format!("请求取消运行中打标任务失败：{error}"))?;
    }
    transaction.commit().map_err(|error| format!("提交取消打标任务失败：{error}"))?;
    read_job(database, id)?.ok_or_else(|| "打标任务不存在".into())
}

fn caption_loop(database_path: &Path, app_data_dir: &Path, app: &AppHandle, stopping: &AtomicBool, wake_signal: &(Mutex<bool>, Condvar)) {
    let Ok(mut database) = Connection::open(database_path) else { return; };
    let _ = database.execute_batch("PRAGMA journal_mode=WAL; PRAGMA foreign_keys=ON; PRAGMA busy_timeout=5000;");
...
fn claim_next_job(database: &mut Connection, app_data_dir: &Path) -> Result<Option<CaptionExecution>, String> {
    let transaction = database.transaction().map_err(|error| format!("开启打标调度事务失败：{error}"))?;
    ...
    let Some(id) = id else { transaction.commit().map_err(|error| format!("提交空闲打标事务失败：{error}"))?; return Ok(None); };
    let now = Utc::now().to_rfc3339();
    ...
    transaction.commit().map_err(|error| format!("提交打标任务领取失败：{error}"))?;
    Ok(Some(execution))
}

fn execute_job(database: &Connection, app_data_dir: &Path, app: &AppHandle, stopping: &AtomicBool, job: CaptionExecution) {
...
enum ExecutionStop { Application, Cancelled, Failed(String) }

fn execute_runner(database: &Connection, app_data_dir: &Path, app: &AppHandle, stopping: &AtomicBool, job: &CaptionExecution) -> Result<(), ExecutionStop> {
    if job.items.is_empty() { return Ok(()); }
    ...
    fs::create_dir_all(&request_directory).map_err(|error| ExecutionStop::Failed(format!("创建打标请求目录失败：{error}")))?;
    ...
    let mut stdout = BufReader::new(child.stdout.take().ok_or_else(|| ExecutionStop::Failed("离线打标组件没有标准输出".into()))?);
    let expected: HashMap<_, _> = job.items.iter().map(|item| (item.asset_id.as_str(), item)).collect();
    let mut line = String::new();
    loop {
        line.clear();
        let read = stdout.read_line(&mut line).map_err(|error| ExecutionStop::Failed(format!("读取离线打标结果失败：{error}")))?;
        if read == 0 { break; }
        if line.len() > MAX_RUNNER_LINE_BYTES { let _ = child.kill(); return Err(ExecutionStop::Failed("离线打标组件单条结果超过限制".into())); }
        if stopping.load(Ordering::SeqCst) { let _ = child.kill(); let _ = child.wait(); let _ = fs::remove_file(&request_path); return Err(ExecutionStop::Application); }
        if cancel_requested(database, &job.id) { let _ = child.kill(); let _ = child.wait(); let _ = fs::remove_file(&request_path); return Err(ExecutionStop::Cancelled); }
        let result: RunnerResult = serde_json::from_str(line.trim()).map_err(|_| ExecutionStop::Failed("离线打标组件返回了无效结果".into()))?;
        let item = expected.get(result.asset_id.as_str()).ok_or_else(|| ExecutionStop::Failed("离线打标组件返回了任务外图片".into()))?;
        apply_runner_result(database, job, item, result).map_err(ExecutionStop::Failed)?;
        emit_job(database, app, &job.id);
    }
    let status = child.wait().map_err(|error| ExecutionStop::Failed(format!("等待离线打标组件退出失败：{error}")))?;
    let stderr = stderr_reader.join().unwrap_or_default();
    let _ = fs::remove_file(&request_path);
    if stopping.load(Ordering::SeqCst) { return Err(ExecutionStop::Application); }
    if cancel_requested(database, &job.id) { return Err(ExecutionStop::Cancelled); }
    if !status.success() { return Err(ExecutionStop::Failed(redact_runner_error(&stderr, &component, &request_path, status.code()))); }
    Ok(())
}

fn apply_runner_result(database: &Connection, job: &CaptionExecution, item: &CaptionExecutionItem, result: RunnerResult) -> Result<(), String> {
    let now = Utc::now().to_rfc3339();
    let transaction = database.unchecked_transaction().map_err(|error| format!("开启逐图打标事务失败：{error}"))?;
    ...
    transaction.commit().map_err(|error| format!("提交逐图打标结果失败：{error}"))
}

fn finish_from_items(database: &Connection, app: &AppHandle, id: &str) {
    let now = Utc::now().to_rfc3339();
    let transaction = match database.unchecked_transaction() { Ok(value) => value, Err(_) => return };
    ...
    let failed: u32 = transaction.query_row("SELECT failed_assets FROM local_caption_jobs WHERE id=?1", [id], |row| row.get(0)).unwrap_or(1);
...
fn select_targets(transaction: &Transaction<'_>, input: &DesktopCaptionJobCreateInput) -> Result<Vec<String>, String> {
    if let Some(asset_id) = &input.asset_id {
        validate_uuid(asset_id, "训练图片 ID")?;
        let exists: bool = transaction.query_row("SELECT EXISTS(SELECT 1 FROM local_training_assets WHERE id=?1 AND dataset_id=?2)", params![asset_id,input.dataset_id], |row| row.get(0)).map_err(|error| format!("读取训练图片失败：{error}"))?;
        return if exists { Ok(vec![asset_id.clone()]) } else { Err("训练图片不存在".into()) };
    }
    ...
    let targets = statement.query_map([&input.dataset_id], |row| row.get(0)).map_err(|error| format!("查询待打标图片失败：{error}"))?.collect::<Result<Vec<_>, _>>().map_err(|error| format!("解析待打标图片失败：{error}"))?;
    Ok(targets)
}

fn read_execution(transaction: &Transaction<'_>, app_data_dir: &Path, id: &str) -> Result<CaptionExecution, String> {
    ...
    let items = statement.query_map([id], |row| { let relative: String = row.get(1)?; Ok(CaptionExecutionItem { asset_id: row.get(0)?, path: app_data_dir.join(relative), force_replace: row.get::<_, i64>(2)? != 0 }) }).map_err(|error| format!("查询逐图打标任务失败：{error}"))?.collect::<Result<Vec<_>, _>>().map_err(|error| format!("解析逐图打标任务失败：{error}"))?;
    Ok(CaptionExecution { id: id.into(), dataset_id, general_threshold, character_threshold, include_character_tags, items })
}
...
fn read_job(database: &Connection, id: &str) -> Result<Option<DesktopCaptionJobView>, String> {
    ...
    let Some(mut job) = row else { return Ok(None); };
    ...
    job.items = statement.query_map([id], |row| Ok(DesktopCaptionJobItemView { asset_id: row.get(0)?, status: row.get(1)?, caption: row.get(2)?, error: row.get(3)? })).map_err(|error| format!("查询逐图打标状态失败：{error}"))?.collect::<Result<Vec<_>, _>>().map_err(|error| format!("解析逐图打标状态失败：{error}"))?;
    Ok(Some(job))
}
...
fn refresh_job_counts(database: &Connection, id: &str, now: &str) -> Result<(), String> {
    ...
    let progress = if total == 0 { 0 } else { processed.saturating_mul(100) / total };
...
fn merge_trigger_words(database: &Connection, dataset_id: &str, tags: Vec<String>) -> Result<String, String> {
    let json: String = database.query_row("SELECT trigger_words_json FROM local_training_datasets WHERE id=?1", [dataset_id], |row| row.get(0)).map_err(|error| format!("读取训练触发词失败：{error}"))?;
    let triggers: Vec<String> = serde_json::from_str(&json).map_err(|error| format!("解析训练触发词失败：{error}"))?;
    let mut seen = HashSet::new();
    let merged = triggers.into_iter().chain(tags).map(|tag| tag.trim().to_owned()).filter(|tag| !tag.is_empty() && seen.insert(tag.to_lowercase())).collect::<Vec<_>>().join(", ");
    if merged.is_empty() || merged.chars().count() > 10_000 { return Err("自动 Caption 为空或超过 10000 个字符".into()); }
    Ok(merged)
}
...
fn update_dataset_review_status(database: &Connection, dataset_id: &str, now: &str) -> Result<(), String> {
    ...
    let status = if count >= 5 && missing == 0 { "review_ready" } else { "draft" };
...
fn find_captioner_component(runtime_root: &str) -> Result<CaptionerComponent, String> {
    ...
    let mut candidates = fs::read_dir(&components_root).map_err(|_| "离线打标组件尚未安装，请先在资源安装页完成安装".to_string())?.filter_map(Result::ok).filter(|entry| entry.path().is_dir()).collect::<Vec<_>>();
    candidates.sort_by_key(|entry| entry.metadata().and_then(|value| value.modified()).ok());
    for entry in candidates.into_iter().rev() {
        let root = entry.path();
        if root.join(".drawhime-resource.json").is_file() && root.join("runner.py").is_file() && root.join("model.onnx").is_file() && root.join("selected_tags.csv").is_file() && root.join("site-packages").join("onnxruntime").join("__init__.py").is_file() {
            return Ok(CaptionerComponent { python, runner: root.join("runner.py"), root });
        }
    }
...
fn write_request(path: &Path, request: &RunnerRequest) -> Result<(), String> {
    ...
    let mut file = File::create(&temporary).map_err(|error| format!("创建打标请求失败：{error}"))?;
    serde_json::to_writer(&mut file, request).map_err(|error| format!("序列化打标请求失败：{error}"))?;
    file.flush().and_then(|_| file.sync_all()).map_err(|error| format!("保存打标请求失败：{error}"))?;
...
fn read_limited_stderr(stderr: Option<impl Read>) -> String {
    ...
    let _ = stderr.by_ref().take(MAX_RUNNER_ERROR_BYTES as u64).read_to_end(&mut bytes);
    String::from_utf8_lossy(&bytes).into_owned()
}

fn redact_runner_error(stderr: &str, component: &CaptionerComponent, request_path: &Path, code: Option<i32>) -> String {
    let mut text = stderr.replace(component.root.to_string_lossy().as_ref(), "<CAPTIONER>").replace(request_path.to_string_lossy().as_ref(), "<REQUEST>");
    text = text.lines().last().unwrap_or_default().trim().chars().take(500).collect();
    if text.is_empty() { format!("离线打标组件退出码 {}", code.unwrap_or(-1)) } else { format!("离线打标组件失败：{text}") }
}

fn validate_create_input(input: &DesktopCaptionJobCreateInput) -> Result<(), String> {
    validate_uuid(&input.dataset_id, "训练集 ID")?;
    if !(0.05..=0.95).contains(&input.general_threshold) || !(0.05..=0.99).contains(&input.character_threshold) { return Err("自动打标阈值超出允许范围".into()); }
    Ok(())
}

fn validate_runner_tags(tags: Vec<String>) -> Result<Vec<String>, String> {
    if tags.is_empty() || tags.len() > MAX_CAPTION_TAGS { return Err("离线打标结果为空或标签数量超过限制".into()); }
    if tags.iter().any(|tag| tag.trim().is_empty() || tag.chars().count() > 200 || tag.contains(['\r','\n','\0'])) { return Err("离线打标结果包含无效标签".into()); }
    Ok(tags)
}

fn sanitize_item_error(error: &str) -> String { error.lines().last().unwrap_or("离线打标失败").trim().chars().take(500).collect() }
fn load_runtime_root(database: &Connection) -> Result<String, String> { database.query_row("SELECT runtime_root FROM desktop_settings WHERE id=1", [], |row| row.get(0)).map_err(|error| format!("读取 Runtime 目录失败：{error}")) }
fn cancel_requested(database: &Connection, id: &str) -> bool { database.query_row("SELECT cancel_requested FROM local_caption_jobs WHERE id=?1", [id], |row| row.get::<_, i64>(0)).map(|value| value != 0).unwrap_or(true) }
fn emit_job(database: &Connection, app: &AppHandle, id: &str) { if let Ok(Some(job)) = read_job(database, id) { let _ = app.emit("desktop-caption-job-updated", job); } }
...
mod tests {
    ...
    fn batch_caption_job_preserves_manual_caption_and_applies_trigger_words() {
        ...
        state.update_training_caption(DesktopTrainingCaptionUpdateInput { dataset_id: dataset.id.clone(), asset_id: imported.assets[0].id.clone(), caption: Some("manual identity".into()) }).expect("保存人工 Caption");
        let job = { let mut database = state.database.lock().expect("锁定数据库"); create_job(&mut database, DesktopCaptionJobCreateInput { dataset_id: dataset.id.clone(), asset_id: None, general_threshold: 0.35, character_threshold: 0.85, include_character_tags: false }).expect("创建批量打标任务") };
        assert_eq!(job.items.len(), 4);
        assert!(!job.items.iter().any(|item| item.asset_id == imported.assets[0].id));
        let item = CaptionExecutionItem { asset_id: job.items[0].asset_id.clone(), path: PathBuf::new(), force_replace: false };
        let execution = CaptionExecution { id: job.id.clone(), dataset_id: dataset.id.clone(), general_threshold: 0.35, character_threshold: 0.85, include_character_tags: false, items: vec![item.clone()] };
        { let database = state.database.lock().expect("锁定数据库"); database.execute("UPDATE local_caption_job_items SET status='running' WHERE job_id=?1 AND asset_id=?2", params![job.id,item.asset_id]).expect("模拟领取任务"); apply_runner_result(&database, &execution, &item, RunnerResult { asset_id: item.asset_id.clone(), tags: Some(vec!["1girl".into(), "solo".into()]), error: None }).expect("写入自动 Caption"); }
        let restored = state.list_training_datasets().expect("恢复训练集");
        assert_eq!(restored[0].assets[0].caption.as_deref(), Some("manual identity"));
        let tagged = restored[0].assets.iter().find(|asset| asset.id == item.asset_id).expect("读取自动打标图片");
