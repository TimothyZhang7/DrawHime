...
import { parseDownloadRange, resolveLoraUploadDirectory } from "./lora-library.js";
...
test("Linux LoRA 上传临时目录必须位于 data 盘", () => {
  assert.equal(resolveLoraUploadDirectory("/data/local-platform/lora-uploads", "linux"), "/data/local-platform/lora-uploads");
  assert.throws(() => resolveLoraUploadDirectory("/tmp/lora-uploads", "linux"), /data 盘/);
  assert.throws(() => resolveLoraUploadDirectory(undefined, "linux"), /尚未配置/);
});
