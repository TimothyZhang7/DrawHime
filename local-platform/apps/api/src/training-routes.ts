...
/** 服务端再次校验模型训练能力，阻止旧页面把蒸馏推理底模提交到训练 Runtime。 */
async function findTrainingModel(id: string) { const model = await database.modelVersion.findFirst({ where: { id, status: "ACTIVE", runtimeFormat: "anima" } }); if (!model || readObject(model.defaultParameters).trainingSupported === false) throw new TrainingRouteError(400, "training_model_invalid", "所选基础模型不支持 Anima LoRA 训练"); return model; }
