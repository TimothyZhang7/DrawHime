...
| LoRA 打标工具 | `tools/lora-captioning-contracts.ts` | 主站工具页通过同源 `/local-model-api` 身份交换访问独立平台训练集、图片、全量或单图自动打标、标签保存、确认、翻译结果、训练集 ZIP 与触发词规则；触发词会原子写入每张图片 Caption，汇总同时返回精确公共标签、同义归一化稳定共识标签与用户触发词并集，训练集数据仍由独立平台权威管理 |
