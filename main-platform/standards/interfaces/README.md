...
- comfy/lora-resumable-sync：ComfyUI 自定义节点保留 `GET/PUT /aiimage/loras/:fileName` 完整文件兼容接口，并新增 `GET/PUT/POST/DELETE /aiimage/loras/:fileName/upload` 断点续传接口。调用方必须传服务 token、完整文件 SHA-256 和总字节数；分片追加还需传当前偏移，节点只接受内容哈希安全文件名、按真实偏移串行追加，完成时校验总大小、SHA-256 与 safetensors 结构后原子替换并刷新 LoRA 缓存。成功响应使用 `{ ok: true, data }`，偏移冲突返回当前真实偏移，失败响应使用 `{ ok: false, code, message }`。
