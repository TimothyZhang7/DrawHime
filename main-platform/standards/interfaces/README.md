...
| LoRA 打标工具 | `tools/lora-captioning-contracts.ts` | 主站工具页通过同源 `/local-model-api` 身份交换访问独立平台训练集、图片、全量或单图自动打标、标签保存、确认、翻译结果和训练集 ZIP；ZIP 流式包含每张图片及同名 UTF-8 Tag 文件，训练集数据仍由独立平台权威管理 |
