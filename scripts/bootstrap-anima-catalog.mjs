...
// 模型文件名、哈希和推荐参数来自对应 Civitai 已发布版本，Runtime 只引用校验后落到 GPU 的真实文件。
const modelCatalog = [
  {
    workflowVersion: 1,
    ...
    sha256: null,
    byteSize: 4182218328,
    parameters: { steps: 8, cfg: 1, sampler: "er_sde", scheduler: "simple", qualityPrefix: "masterpiece, best quality, score_7", defaultNegativePrompt, systemHighresLoraEnabled: true },
  },
  {
    workflowVersion: 2,
    ...
    byteSize: 4182218504,
    parameters: { steps: 30, cfg: 4, sampler: "er_sde", scheduler: "simple", qualityPrefix: "masterpiece, best quality, score_7, very aesthetic", defaultNegativePrompt: `${defaultNegativePrompt}, blurry, jpeg artifacts, sepia, muscular female`, systemHighresLoraEnabled: false },
  },
  {
    workflowVersion: 3,
    ...
    byteSize: 4182218328,
    parameters: { steps: 30, cfg: 4.5, sampler: "euler", scheduler: "normal", qualityPrefix: "best quality, score_7, score_9, very aesthetic, ultra detailed, fair skin, high contrast, photorealistic, raw photo, photo background", defaultNegativePrompt, systemHighresLoraEnabled: false },
  },
  {
    workflowVersion: 4,
    ...
    byteSize: 4182218328,
    parameters: { steps: 30, cfg: 4.5, sampler: "euler_ancestral", scheduler: "normal", qualityPrefix: "best quality, score_7, score_9, very aesthetic, ultra detailed, high contrast", defaultNegativePrompt, systemHighresLoraEnabled: false },
  },
];
...
try {
  ...
  for (const catalog of modelCatalog) {
    ...
    // 已有模型只补缺失字段，管理员明确修改过的价格、采样参数和开关保持不变。
    if (Object.keys(missingDefaults).length > 0) await database.modelVersion.update({ where: { id: model.id }, data: { defaultParameters: { ...modelDefaults, ...missingDefaults } } });
    const effectiveDefaults = { ...defaults, ...modelDefaults };
    ...
    const workflowJson = {
      ...
      sampling: { steps: effectiveDefaults.steps, cfg: effectiveDefaults.cfg, sampler: effectiveDefaults.sampler, scheduler: effectiveDefaults.scheduler },
      ...
      output: { format: "WEBP", quality: 100 },
    };
